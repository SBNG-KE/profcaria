...
export default function EmployerLayoutContent({ children }: { children: ReactNode }) {
  ...
  const sidebar = <aside className={`flex h-full flex-col border-r-2 border-[var(--text-primary)] bg-[var(--surface-raised)] transition-[width] ${collapsed ? 'w-[76px]' : 'w-[264px]'}`}>
    <div className="flex h-16 items-center justify-between border-b border-[var(--border-primary)] px-4"><Link href="/" className="flex items-center gap-2 overflow-hidden font-editorial text-lg font-semibold"><span className="profcaria-mark grid h-8 w-8 shrink-0 place-items-center text-[10px] text-[var(--accent-primary)]"><span className="relative z-10 text-[var(--text-inverse)]">PC</span></span>{!collapsed && <span>Profcaria</span>}</Link><button onClick={toggleCollapsed} className="hidden lg:block" aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{collapsed ? <ChevronRight size={16} /> : <ChevronLeft size={16} />}</button></div>
