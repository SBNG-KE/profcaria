...
export default function EmployerProfileViewModal({
    ...
    return (
        <div className="fixed inset-0 z-[150] flex h-screen w-screen bg-black/80 backdrop-blur-sm overflow-hidden animate-in fade-in duration-300">
            ...
            <div className={`relative w-full h-full flex flex-col md:flex-row ${isDark ? 'bg-black text-white' : 'bg-[#FBF8F0] text-black'}`}>
                ...
                <aside className={`hidden md:flex w-80 flex-shrink-0 h-full overflow-y-auto border-r p-6 flex-col ${isDark ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200 shadow-xl z-20'}`}>
                    ...
                    {intentMode && intentMode !== 'not_looking' && (
                        <div className={`flex items-center gap-2 px-3 py-2 rounded-xl mb-2 ${intentMode === 'actively_looking' ? 'bg-[#738D7D]/10 border border-[#738D7D]/20' :
                                'bg-blue-500/10 border border-blue-500/20'
                            }`}>
                            <div className="relative flex h-2 w-2">
                                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${intentMode === 'actively_looking' ? 'bg-[#9FB5A8]' : 'bg-blue-400'
                                    }`}></span>
                                <span className={`relative inline-flex rounded-full h-2 w-2 ${intentMode === 'actively_looking' ? 'bg-[#738D7D]' : 'bg-blue-500'
                                    }`}></span>
                            </div>
                            <span className={`text-[10px] font-black uppercase tracking-widest ${intentMode === 'actively_looking' ? 'text-[#738D7D]' : 'text-blue-500'
                                }`}>
                                    ...
                                    {verification ? (
                                        <div className="space-y-3">
                                            {verification.checks.map((check, i) => (
                                                <div key={i} className={`flex items-center gap-4 p-3 sm:p-4 rounded-2xl border transition-all ${check.status === 'verified' ? (isDark ? 'border-[#738D7D]/30 bg-[#738D7D]/10' : 'border-[#9FB5A8] bg-[#738D7D]/5') :
                                                        check.status === 'partial' ? (isDark ? 'border-[#738D7D]/30 bg-[#738D7D]/10' : 'border-[#9FB5A8] bg-[#738D7D]/5') :
                                                            isDark ? 'border-neutral-800 bg-neutral-800/50' : 'border-neutral-200 bg-neutral-50'
                                                    }`}>
                                                    {check.status === 'verified' ? (
                                                        <CheckCircle2 size={20} className="text-[#738D7D] shrink-0" />
                                                    ) : check.status === 'partial' ? (
                                                        <Clock size={20} className="text-[#738D7D] shrink-0" />
                                                    ) : (
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className={`text-xs ${check.status === 'verified' ? (isDark ? 'text-[#9FB5A8]' : 'text-[#738D7D]') :
                                                                check.status === 'partial' ? (isDark ? 'text-[#9FB5A8]' : 'text-[#738D7D]') :
                                                                    isDark ? 'text-neutral-500' : 'text-neutral-400'
                                                            }`}>{check.detail}</p>
                                                    </div>
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider shrink-0 ${check.status === 'verified' ? 'bg-[#738D7D]/10 text-[#738D7D]' :
                                                            check.status === 'partial' ? 'bg-[#738D7D]/10 text-[#738D7D]' :
                        ...
                        {activeTab === 'profile' && (
                            <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-1.5 rounded-lg transition-all ${copiedField === 'email' ? 'bg-[#738D7D]/10 text-[#738D7D]' : 'hover:bg-neutral-800 text-neutral-500 hover:text-white'}`}
                                                        title="Copy Email"
                                                    >
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {data.profile.phone && (
                                                        <button
                                                            ...
                                                            className={`p-1.5 rounded-lg transition-all ${copiedField === 'phone' ? 'bg-[#738D7D]/10 text-[#738D7D]' : 'hover:bg-neutral-800 text-neutral-500 hover:text-white'}`}
                                                            title="Copy Phone"
                                                        >
                                ...
                                {data.kycData && (data.kycData.imageUrl || data.kycData.videoUrl) && (
                                    <div className={`p-6 sm:p-10 rounded-[24px] sm:rounded-[40px] border-2 ${isDark ? 'bg-[#738D7D]/5 border-[#738D7D]/20' : 'bg-[#738D7D]/5 border-[#738D7D]/20 shadow-sm'}`}>
                                        <h3 className={`text-xs font-black uppercase tracking-widest mb-4 sm:mb-6 flex items-center gap-2 ${isDark ? 'text-[#738D7D]' : 'text-[#738D7D]'}`}>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            {data.kycData.imageUrl && (
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="rounded-2xl overflow-hidden border border-[#738D7D]/30 aspect-[4/3] bg-black">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex items-start justify-between">
                                                                        ...
                                                                        {job.source === 'automatic' && (
                                                                            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border ${isDark ? 'bg-[#738D7D]/10 border-[#738D7D]/20 text-[#738D7D]' : 'bg-[#738D7D]/5 border-[#738D7D]/20 text-[#738D7D]'}`} title="Verified via Profcaria Connection">
                            ...
                            return (
                                <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
                                    ...
                                    <div className={`p-4 sm:p-8 rounded-[20px] sm:rounded-[32px] border mb-4 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                                        ...
                                        <div className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[8px] sm:text-[10px] font-black uppercase tracking-widest whitespace-nowrap ${showUploaded ? (isDark ? 'bg-[#738D7D]/10 text-[#738D7D]' : 'bg-[#738D7D]/5 text-[#738D7D]') : (isDark ? 'bg-[#738D7D]/10 text-[#738D7D]' : 'bg-[#738D7D]/5 text-[#738D7D]')}`}>
                                        ...
                                        <div className="space-y-3">
                                            {sentRequests.map((req) => {
                                                const getStatusBadge = () => {
                                                    switch (req.status) {
                                                        case 'responded':
                                                            return { label: 'Responded', color: isDark ? 'text-[#9FB5A8] bg-[#738D7D]/20' : 'text-[#738D7D] bg-[#738D7D]/5' };
                                                        case 'viewed':
                                        ...
                                        {verifiedEmployments.map((emp) => {
                                            ...
                                            const getTerminationInfo = () => {
                                                switch (emp.terminationType) {
                                                    case 'resignation':
                                                        return { icon: <LogOut size={14} />, label: 'Resigned', color: isDark ? 'text-[#9FB5A8] bg-[#738D7D]/10 border-[#738D7D]/20' : 'text-[#738D7D] bg-[#738D7D]/5 border-[#738D7D]/20' };
                                                    case 'involuntary':
                                                ...
                                                <div key={emp.id} className={`p-6 sm:p-8 rounded-[24px] sm:rounded-[32px] border ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200 shadow-sm'}`}>
                                                    <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
                                                        ...
                                                        <div className="flex-1 space-y-3">
                                                            ...
                                                            {emp.companyEmail && (
                                                                <div className="mt-4 pt-4 border-t flex justify-end" style={{ borderColor: isDark ? '#153027' : '#F4F0E6' }}>
                                                                    <button
                                                                        onClick={() => {
