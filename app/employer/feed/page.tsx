...
const CommentModal = ({ isOpen, onClose, postId, isDark, onCommentAdded }: { isOpen: boolean, onClose: () => void, postId: string, isDark: boolean, onCommentAdded: () => void }) => {
        ...
        <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center">
            ...
            <div className={`relative w-full max-w-lg max-h-[80vh] flex flex-col rounded-t-3xl sm:rounded-2xl overflow-hidden ${isDark ? 'bg-neutral-900' : 'bg-white'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {isLoading ? (
                        <div className="flex justify-center py-8"><div className={`animate-spin w-6 h-6 border-2 border-t-transparent rounded-full ${isDark ? 'border-white' : 'border-black'}`} /></div>
                    ) : comments.length === 0 ? (
                        <p className={`text-center py-8 text-sm ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>No comments yet. Be the first!</p>
                    ) : (
                        comments.map((c) => (
                            <div key={c.id} className="flex gap-3">
                                ...
                                <div className="flex-1">
                                    <div className={`px-3 py-2 rounded-xl ${isDark ? 'bg-neutral-800' : 'bg-neutral-100'}`}>
                                        ...
                                        <p className={`text-sm mt-0.5 ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>{c.content}</p>
                                    </div>
                                    <p className={`text-xs mt-1 ml-3 ${isDark ? 'text-neutral-600' : 'text-neutral-400'}`}>{new Date(c.createdAt).toLocaleDateString()}</p>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div className={`p-4 border-t flex gap-2 ${isDark ? 'border-neutral-800' : 'border-neutral-200'}`}>
                    <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Write a comment..." onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} className={`flex-1 px-4 py-2 rounded-full text-sm focus:outline-none ${isDark ? 'bg-neutral-800 text-white placeholder-neutral-500' : 'bg-neutral-100 text-black placeholder-neutral-400'}`} />
                    <button onClick={handleSubmit} disabled={!newComment.trim() || isSending} className={`p-2.5 rounded-full transition-all disabled:opacity-50 ${isDark ? 'bg-white text-black' : 'bg-black text-white'}`}><Send size={16} /></button>
                </div>
            </div>
        </div>
    );
...
// Post Card - Horizontal Layout for Employers (No Follow/Join)
const PostCard = ({ post, isDark, currentUserId, onLike, onRepost, onShare, onReport, onDelete, onEdit, onCommentAdded }: {
    post: any,
    isDark: boolean,
    ...
    const handleSubmitComment = async () => {
        ...
        setIsSending(true);
        try {
            const res = await fetch(`/api/professional/posts/${post.id}/comments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: newComment.trim() }) });
            if (res.ok) { setNewComment(''); fetchComments(); onCommentAdded(); }
        } catch (err) { console.error(err); } finally { setIsSending(false); }
    };
        ...
        <div className={`rounded-xl border overflow-hidden ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200 shadow-sm'}`}>
            <div className="flex flex-col sm:flex-row">
                        ...
                        <div className="relative overflow-hidden bg-black/5 dark:bg-white/5 flex items-center justify-center min-h-[250px] sm:min-h-[300px] max-h-[600px]">
                            {post.media && post.media.length > 0 ? (
                                <>
                                    ...
                                    {post.media.length > 1 && <div className={`absolute top-3 right-3 z-20 px-2 py-1 rounded-lg text-xs font-bold ${isDark ? 'bg-black/70 text-white' : 'bg-white/90 text-black'}`}>+{post.media.length - 1}</div>}
                                </>
                            ) : post.linkPreview ? (
                ...
                <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${showComments && (hasMedia || post.linkPreview) ? 'sm:max-w-[30%]' : ''}`}>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                <div className={`flex items-center gap-1.5 text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>
                                    ...
                                    {!isProfessional && post.author.location && <span className="flex items-center gap-0.5 truncate max-w-[100px]"><MapPin size={8} />{post.author.location}</span>}
                    ...
                    <div className={`px-4 py-2 flex-1 ${showComments ? 'min-h-[100px]' : 'min-h-[200px]'}`}><ScrollableText text={post.content} isDark={isDark} /></div>
                    {(post.likesCount > 0 || post.commentsCount > 0 || post.repostsCount > 0) && (
                        <div className={`px-3 py-1.5 flex items-center justify-between text-[10px] ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>
                            <div className="flex items-center gap-2">{post.likesCount > 0 && <span className="flex items-center gap-1"><span className="w-3.5 h-3.5 rounded-full bg-red-500 flex items-center justify-center"><Heart size={7} className="text-white" fill="white" /></span>{post.likesCount}</span>}</div>
                ...
                {showComments && (
                    <div className={`w-full sm:w-[35%] sm:min-w-[280px] flex flex-col border-t sm:border-t-0 sm:border-l transition-all duration-300 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-neutral-50 border-neutral-200'}`}>
                        ...
                        <div className={`p-3 border-b flex gap-2 ${isDark ? 'border-neutral-800' : 'border-neutral-200'}`}>
                            <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Write a comment..." onKeyDown={(e) => e.key === 'Enter' && handleSubmitComment()} className={`flex-1 px-3 py-2 rounded-lg text-sm focus:outline-none ${isDark ? 'bg-neutral-800 text-white placeholder-neutral-500' : 'bg-white text-black placeholder-neutral-400 border border-neutral-200'}`} />
                        ...
                        <div className="flex-1 overflow-y-auto p-3 space-y-3">
                            {isLoadingComments ? <div className="flex justify-center py-4"><div className={`animate-spin w-5 h-5 border-2 border-t-transparent rounded-full ${isDark ? 'border-white' : 'border-black'}`} /></div>
...
const PostCreationModal = ({ isOpen, onClose, isDark, onPost, initialData }: {
    ...
    const handlePost = () => {
        ...
        onPost({ content: content.trim(), mediaUrls: images, linkMedia: linkMedia || undefined });
...
export default function EmployerFeedPage() {
    ...
    const [commentModalPost, setCommentModalPost] = useState<string | null>(null);
    ...
    const handleDeletePost = async (postId: string) => {
        ...
        try {
            ...
            if (!res.ok) { fetchPosts(); alert('Failed to delete post'); }
        ...
        <div className="min-h-full pb-20 relative">
            ...
            <div className="max-w-4xl mx-auto space-y-4 px-4">
                {isLoading ? (
                    ...
                    <PostCard key={post.id} post={post} isDark={isDark} currentUserId={currentUserId} onLike={() => handleLike(post.id)} onRepost={() => handleRepost(post.id)} onShare={() => handleShare(post.id)} onReport={handleReport} onDelete={handleDeletePost} onEdit={handleStartEdit} onCommentAdded={fetchPosts} />
                ))}
            </div>
