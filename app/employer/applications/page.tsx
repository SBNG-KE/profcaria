...
function ApplicationsPageContent() {
    ...
    const handleOpenMessages = () => {
        if (selectedApp) {
            router.push(`/employer/notifications?applicationId=${selectedApp.id}`);
        }
    };
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="shrink-0 flex flex-col items-end gap-1">
                                            <span className={`w-2 h-2 rounded-full ${app.status === 'pending' ? 'bg-[#738D7D]' :
                                                app.status === 'pending_verification' ? 'bg-[#738D7D] animate-pulse' :
                                                    app.status === 'shortlisted' ? 'bg-blue-500' :
                                                        app.status === 'employed' ? 'bg-[#738D7D]' : (isDark ? 'bg-neutral-500' : 'bg-neutral-300')
                                                }`} />
                                        </div>
                                    </div>
                                </button>
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4 mt-2">
                                            ...
                                            <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border ${selectedApp.status === 'pending' ? 'bg-[#738D7D]/10 text-[#738D7D] border-[#738D7D]/20' :
                                                selectedApp.status === 'shortlisted' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                                                    selectedApp.status === 'employed' ? 'bg-[#738D7D]/10 text-[#738D7D] border-[#738D7D]/20' :
                                            ...
                                            <div className="flex flex-wrap gap-4">
                                                {selectedApp.status === 'pending' && (
                                                    <>
                                                        <button
                                                            ...
                                                            className="px-6 py-3 bg-[#738D7D] hover:bg-[#738D7D] text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-[#738D7D]/20 active:scale-95 flex items-center gap-2"
                                                        >
                                                ...
                                                {selectedApp.status === 'pending_verification' && (
                                                    <div className="w-full space-y-3">
                                                        <div className={`flex items-center gap-3 p-4 rounded-xl border ${isDark ? 'bg-[#738D7D]/10 border-[#738D7D]/20' : 'bg-[#738D7D]/5 border-[#738D7D]/20'}`}>
                                                            <div className="relative flex h-3 w-3 shrink-0">
                                                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#9FB5A8] opacity-75"></span>
                                                                <span className="relative inline-flex rounded-full h-3 w-3 bg-[#738D7D]"></span>
                                                            </div>
                                                            <div>
                                                                <p className={`text-xs font-black uppercase tracking-widest ${isDark ? 'text-[#9FB5A8]' : 'text-[#738D7D]'}`}>Awaiting Identity Verification</p>
                                                                <p className={`text-[10px] font-medium mt-1 ${isDark ? 'text-[#9FB5A8]/60' : 'text-[#738D7D]/70'}`}>
                                                ...
                                                {selectedApp.status === 'employed' && (
                                                    <div className="w-full flex items-center justify-center gap-2 text-[#738D7D] text-sm font-black uppercase tracking-widest py-2">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className={`relative border rounded-[32px] p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="text-center mb-6">
                            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[#738D7D]/10 border border-[#738D7D]/20 flex items-center justify-center">
                                <XCircle className="text-[#738D7D]" size={32} />
                            </div>
                        ...
                        <div className="bg-[#738D7D]/10 border border-[#738D7D]/20 rounded-xl p-4 mb-6">
                            <p className="text-[#738D7D] text-sm text-center font-medium">
