...
export default function EmployerProfilePage() {
    ...
    return (
        <div className="max-w-5xl mx-auto py-8 px-4 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
            {/* Message Banner */}
            {message && (
                <div className={`p-4 rounded-xl text-sm font-medium ${message.type === 'success' ? (isDark ? 'bg-[#ED3EB3]/10 text-[#35D7DC] border border-[#ED3EB3]/20' : 'bg-[#ED3EB3]/10 text-[#ED3EB3] border border-[#ED3EB3]/20') : (isDark ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-red-50 text-red-700 border border-red-200')}`}>
                    {message.text}
                </div>
            )}
            ...
            <div className={`p-8 rounded-[40px] ${isDark ? 'bg-neutral-900 border border-neutral-800' : 'bg-white border-neutral-200 shadow-sm'}`}>
                <div className="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 w-full space-y-6">
                        ...
                        <div className="space-y-2 w-full max-w-full overflow-hidden">
                            ...
                            <div className={`grid grid-cols-[1fr_auto] items-center p-1.5 rounded-xl border w-full max-w-full overflow-hidden ${isDark ? 'bg-neutral-950 border-neutral-800' : 'bg-neutral-50 border-neutral-200'}`}>
                                <div className={`px-3 text-sm truncate min-w-0 ${isDark ? 'text-neutral-400' : 'text-black'}`}>
                                    {typeof window !== 'undefined' ? `${window.location.origin}/${profile?.shortUrl || (companyName || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}` : '...'}
                                </div>
                                <button
                                    onClick={async () => {
                                        try {
                                            const res = await fetch(`/api/share?type=profile&userType=employer&id=${profile?.id}`);
                                            let link = `${window.location.origin}/${profile?.shortUrl || (companyName || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`;
                                            if (res.ok) {
                                                const data = await res.json();
                                                link = data.link;
                                            }
                                            ...
                                            navigator.clipboard.writeText(`${window.location.origin}/${profile?.shortUrl || (companyName || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`);
                                            setMessage({ type: 'success', text: 'Profile link copied!' });
                                        }
                                ...
                                >
                                    {message?.text === 'Profile link copied!' ? <Check size={16} className="text-[#ED3EB3]" /> : <Copy size={16} />}
                                </button>
                            </div>
                        ...
                        {/* Careers Page Link */}
                        <div className="space-y-2 w-full max-w-full overflow-hidden">
                            <label className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-neutral-500' : 'text-neutral-600'}`}>Careers Page</label>
                            <div className={`grid grid-cols-[1fr_auto] items-center p-1.5 rounded-xl border w-full max-w-full overflow-hidden ${isDark ? 'bg-neutral-950 border-neutral-800' : 'bg-neutral-50 border-neutral-200'}`}>
                                <div className={`px-3 text-sm truncate min-w-0 ${isDark ? 'text-neutral-400' : 'text-black'}`}>
                                    {typeof window !== 'undefined' ? `${window.location.origin}/company/${(companyName || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}/careers` : '...'}
                                </div>
                                <button
                                    onClick={() => {
                                        ...
                                        setMessage({ type: 'success', text: 'Careers page link copied!' });
                                        setTimeout(() => setMessage(null), 3000);
                                    }}
                                    className={`p-2 rounded-lg transition-colors shrink-0 ${isDark ? 'hover:bg-neutral-800 text-white' : 'hover:bg-white text-black shadow-sm'}`}
                                >
                                    {message?.text === 'Careers page link copied!' ? <Check size={16} className="text-[#ED3EB3]" /> : <Copy size={16} />}
                                </button>
                            </div>
