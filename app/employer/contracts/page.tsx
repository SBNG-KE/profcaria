            ...
            <div className="max-w-7xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-wrap gap-4 items-center justify-between">
                    <div className="relative flex-1 min-w-[300px]">
                        ...
                        <input type="text" placeholder="Search by professional or job title..." className="w-full bg-[#061D20]/50 border border-white/5 rounded-2xl pl-12 pr-4 py-3 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-[#ED3EB3]/50 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                </div>
                ...
                {isLoading ? (
                    <div className="p-12 text-center">
                        <div className="animate-spin w-8 h-8 border-2 border-[#ED3EB3] border-t-transparent rounded-full mx-auto mb-4"></div>
                        <p className="text-slate-500">Loading contracts...</p>
                    </div>
                ) : filteredContracts.length === 0 ? (
                    <div className="bg-[#061D20] border border-slate-800 rounded-[40px] p-12 text-center space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredContracts.map(contract => (
                            <div key={contract.id} className="group bg-[#061D20] border border-slate-800 rounded-[32px] p-6 hover:border-[#ED3EB3]/30 transition-all relative overflow-hidden">
                                <div className="absolute top-0 right-0 p-6 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href={contract.contract_url} target="_blank" rel="noopener noreferrer" className="p-2 bg-[#ED3EB3] text-white rounded-xl shadow-lg hover:bg-[#35D7DC] block" title="Download Contract">
                                        <Download size={18} />
                                    </a>
                                </div>

                                <div className="space-y-4">
                                    <div className="w-12 h-12 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-[#ED3EB3] mb-4">
                                        <FileText size={20} />
                                    </div>
                                    ...
                                    <div className="pt-4 border-t border-slate-800 space-y-2">
                                        ...
                                        <div className="flex justify-between items-center text-xs">
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${contract.status === 'terminated' ? 'bg-red-500/10 text-red-500' : 'bg-[#ED3EB3]/10 text-[#ED3EB3]'}`}>{contract.status}</span>
                                        </div>
                                    </div>
