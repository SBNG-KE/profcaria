        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8 pb-32">
                    ...
                    <div className="flex p-1 bg-slate-800/50 rounded-xl border border-slate-700">
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 ${filterType === 'starred'
                                ? 'bg-[#738D7D] text-black'
                                : 'text-slate-400 hover:text-[#9FB5A8]'
                                }`}
                        >
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
                        ...
                        {filteredApplications.length === 0 && filterType === 'starred' ? (
                            ...
                            filteredApplications.map((app) => (
                                <div
                                    ...
                                    className={`w-full text-left p-6 rounded-[32px] border transition-all duration-300 flex items-center justify-between group cursor-pointer ${selectedApp?.id === app.id ? 'bg-blue-600/10 border-blue-500/50 shadow-2xl' : 'bg-[#0D211A] border-slate-800 hover:border-slate-700'}`}
                                >
                                    <div className="flex items-center gap-5">
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-xl transition-all active:scale-90 ${app.is_starred
                                                ? 'bg-[#738D7D]/20 text-[#9FB5A8] border border-[#738D7D]/30'
                                                : 'bg-slate-800 text-slate-600 border border-slate-700 hover:text-[#9FB5A8] hover:border-[#738D7D]/30'
                                                } ${togglingStarId === app.id ? 'opacity-50 animate-pulse' : ''}`}
                                        >
                                            <Star size={16} className={app.is_starred ? 'fill-[#9FB5A8]' : ''} />
                                        </button>
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {app.wasInvited && (
                                                    <span className="px-2 py-0.5 bg-[#738D7D]/10 border border-[#738D7D]/20 rounded text-[8px] font-black text-[#9FB5A8] uppercase tracking-widest flex items-center gap-1">
                                                        <CheckCircle2 size={10} /> Was Invited
                                                    </span>
                                                )}
                                                {app.is_starred && (
                                                    <span className="px-2 py-0.5 bg-[#738D7D]/10 border border-[#738D7D]/20 rounded text-[8px] font-black text-[#9FB5A8] uppercase tracking-widest">
                                                        Starred
                                                    </span>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${app.status === 'interview_scheduled' ? 'bg-[#738D7D]/10 text-[#9FB5A8]' : 'bg-blue-500/10 text-blue-400'}`}>
                    ...
                    {/* DETAIL AREA */}
                    <div className="sticky top-8 bg-[#0D211A] border border-slate-800 rounded-[40px] p-8 min-h-[600px] flex flex-col overflow-hidden">
                        {selectedApp ? (
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        <button
                                            ...
                                            className="px-4 py-2.5 bg-[#738D7D]/10 border border-[#738D7D]/20 text-[#9FB5A8] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#738D7D]/20 transition-all whitespace-nowrap"
                                        >
                                ...
                                <div className="space-y-6 text-left">
                                    {selectedApp.status === 'employed' && selectedApp.progression && selectedApp.progression.length > 0 && (
                                        <div className="mb-8 space-y-4">
                                            <h3 className="text-sm font-black text-[#9FB5A8] uppercase tracking-widest border-b border-[#738D7D]/20 pb-2 flex items-center gap-2">
                                            ...
                                            <div className="relative pl-4 space-y-6 before:absolute before:inset-y-0 before:left-0 before:w-px before:bg-slate-800">
                                                {selectedApp.progression.map((role: any, idx: number) => (
                                                    <div key={role.id || idx} className="relative">
                                                        <div className="absolute -left-[21px] top-1.5 w-2 h-2 rounded-full bg-[#738D7D] ring-4 ring-[#0D211A]"></div>
                                                        <div className="space-y-1">
                                                            <div className="flex items-center gap-2">
                                                                <h4 className="text-lg font-bold text-white leading-none">{role.title}</h4>
                                                                {role.isCurrent && (
                                                                    <span className="px-2 py-0.5 bg-[#738D7D]/10 text-[#9FB5A8] text-[8px] font-black uppercase tracking-widest rounded">Current</span>
                                                                )}
                                                            </div>
            ...
            {showChat && selectedApp && (
                <div className="fixed inset-0 z-[120] flex items-center justify-end p-4">
                    ...
                    <div className="relative w-full max-w-lg h-[90vh] bg-[#0D211A] border border-slate-700 rounded-[40px] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-right-10 duration-500">
            ...
            {showInterviewModal && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-[#0D211A] border border-slate-700 rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
