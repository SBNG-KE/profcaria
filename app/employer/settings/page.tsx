...
import { User, Save, Shield, MapPin, Globe, Activity, Lock, CheckCircle, CreditCard, LayoutDashboard, Loader2, Star, Users, TrendingUp, Eye, EyeOff, Smartphone, Mail, AlertTriangle, Power } from 'lucide-react';
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                        ...
                        {nextBadgeTier ? (
                            <div>
                                ...
                                <div className={`w-full h-3 rounded-full overflow-hidden ${isDark ? 'bg-neutral-800' : 'bg-neutral-200'}`}>
                                    <div
                                        className={`h-full rounded-full transition-all duration-1000 ease-out ${nextBadgeTier.key === 'gold' ? 'bg-gradient-to-r from-[#ED3EB3] to-[#35D7DC]' : nextBadgeTier.key === 'blue' ? 'bg-gradient-to-r from-blue-500 to-blue-400' : 'bg-gradient-to-r from-neutral-500 to-neutral-400'}`}
                        ...
                        {badgeTiers.map((tier) => {
                            ...
                            return (
                                <div key={tier.key} className={`relative p-6 rounded-3xl border ${isDark ? `bg-neutral-900/50 ${borderColor}` : `bg-white border-neutral-200`} ${activeRing} transition-all`}>
                                    {isCurrent && (
                                        <div className="absolute top-0 right-0 bg-[#ED3EB3] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-bl-xl rounded-tr-3xl">
                                            Current
                                        </div>
                                    )}
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="flex items-start gap-4">
                            <div className="p-3 bg-[#ED3EB3]/10 rounded-xl">
                                <TrendingUp className="text-[#ED3EB3]" size={24} />
                            </div>
                            <div>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                    ...
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="flex items-center justify-between">
                            <h3 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-black'}`}>
                                <Smartphone className={isDark ? 'text-white' : 'text-black'} size={24} /> Multi-Factor Authentication
                            </h3>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${isDark ? 'bg-white/10 text-white border-white/20' : 'bg-black/10 text-black border-black/20'}`}>
                                Verified
                            </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            >
                                <div className="flex items-center gap-3 mb-2">
                                    <Mail size={20} className={mfaMethod === 'email' ? (isDark ? 'text-white' : 'text-black') : isDark ? 'text-neutral-500' : 'text-neutral-400'} />
                                    <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-black'}`}>Email OTP</span>
                                    {mfaMethod === 'email' && <CheckCircle size={16} className={isDark ? 'text-white ml-auto' : 'text-black ml-auto'} />}
                                </div>
                            ...
                            >
                                <div className="flex items-center gap-3 mb-2">
                                    <Smartphone size={20} className={mfaMethod === 'authenticator' ? (isDark ? 'text-white' : 'text-black') : isDark ? 'text-neutral-500' : 'text-neutral-400'} />
                                    <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-black'}`}>Authenticator App</span>
                                    {mfaMethod === 'authenticator' && <CheckCircle size={16} className={isDark ? 'text-white ml-auto' : 'text-black ml-auto'} />}
                                </div>
                                <p className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>Use Google Authenticator or similar app for codes</p>
                            </button>
                        </div>

                        <div className={`flex items-center gap-3 p-4 rounded-xl border ${isDark ? 'border-neutral-600 bg-neutral-800/50' : 'border-neutral-300 bg-neutral-50'}`}>
...
export default function EmployerSettingsPage() {
    return (
        <Suspense fallback={<div className="flex h-screen items-center justify-center bg-[#061D20]"><Loader2 className="text-white animate-spin" size={48} /></div>}>
            <SettingsContent />
        </Suspense>
    );
}
