...
const InterviewCard = ({ interview, onEdit, onJoin, onConnect }: { interview: Interview, onEdit: () => void, onJoin: () => void, onConnect: () => void }) => {
    ...
    return (
        <div className="bg-[#0D211A]/50 border border-white/5 rounded-[32px] p-6 hover:border-violet-500/30 transition-all group relative overflow-hidden">
            ...
            <div className="flex items-center justify-between pt-6 border-t border-white/5">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${interview.status === 'scheduled' ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' :
                    interview.status === 'completed' ? 'bg-[#738D7D]/10 text-[#9FB5A8] border border-[#738D7D]/20' :
                ...
                {isPast ? (
                    <button
                        onClick={onConnect}
                        className="px-4 py-2 bg-[#738D7D] hover:bg-[#738D7D] text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-[#738D7D]/20 transition-all flex items-center gap-2 animate-pulse"
                    >
                        <Users size={12} />
                        Connect Now
                    </button>
        ...
        <div className="p-8 h-full flex flex-col">
            <div className="max-w-7xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                {loading ? (
                    ...
                    <div className="py-32 flex flex-col items-center justify-center text-slate-600 space-y-4 bg-[#0D211A]/30 border border-white/5 rounded-[40px]">
                ...
                {/* Calendar View */}
                <div className="bg-[#0D211A]/20 border border-white/5 rounded-[40px] p-8">
                    ...
                    <div className="grid grid-cols-7 gap-px bg-white/5 border border-white/5 rounded-2xl overflow-hidden">
                        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                            <div key={day} className="bg-[#0D211A] p-4 text-[10px] font-black text-slate-600 uppercase text-center tracking-widest">{day}</div>
                        ))}
                        {Array.from({ length: 35 }).map((_, i) => {
                            ...
                            return (
                                <div key={i} className="bg-[#0D211A]/50 h-32 p-4 border-t border-white/5 hover:bg-white/[0.02] transition-colors relative">
            ...
            {showScheduleModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-2xl bg-[#0D211A] border border-slate-700 rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95">
