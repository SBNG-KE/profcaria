...
export default function EmployerProfileViewPage() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                // Point to the correct profile API endpoint
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#0D211A]">
            <div className="w-12 h-12 border-4 border-[#738D7D]/20 border-t-[#738D7D] rounded-full animate-spin"></div>
        </div>
    );

    if (!data) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#0D211A] space-y-4">
            ...
            <button onClick={() => router.back()} className="text-[#738D7D] font-bold hover:underline uppercase text-xs">Go Back</button>
        </div>
    );
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-black text-white' : 'bg-[#FBF8F0] text-black'} overflow-hidden flex`}>
            ...
            <aside className={`w-80 flex-shrink-0 h-screen overflow-y-auto border-r p-6 flex flex-col ${isDark ? 'bg-neutral-900/50 border-neutral-800' : 'bg-white border-neutral-200 shadow-xl z-20'}`}>
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="w-8 h-8 bg-[#738D7D] rounded-lg flex items-center justify-center">
                    ...
                    <button
                        onClick={() => router.back()}
                        className={`p-2 rounded-lg transition-colors ${isDark ? 'text-neutral-500 hover:bg-neutral-800 hover:text-white' : 'text-neutral-400 hover:bg-neutral-100 hover:text-black'}`}
                        title="Close View"
                    >
                        <X size={18} />
                    </button>
                ...
                <div className="text-center space-y-4 mb-2">
                    <div className="relative mx-auto w-32 h-32 rounded-[2rem] overflow-hidden border-4 border-[#738D7D]/20 shadow-2xl group cursor-default">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-bold text-[#738D7D] uppercase tracking-widest mt-2">{data.profile.role}</p>
                    </div>
                </div>

                {/* Live Badge */}
                <div className="flex justify-center mb-8">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                ...
                <nav className="flex-1 space-y-2">
                    <button
                        className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-bold transition-all bg-[#738D7D] text-white shadow-lg shadow-[#738D7D]/20`}
                    >
                        <User size={20} />
                        Profile
                    </button>
                    {/* Only Profile button visible as requested */}
                </nav>
            ...
            <main className="flex-1 h-screen overflow-y-auto p-8 md:p-12">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    {/* Content Tabs */}
                    <div className={`flex space-x-2 p-1.5 rounded-2xl w-fit border mb-8 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <button
                            onClick={() => setActiveTab('profile')}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'profile' ? (isDark ? 'bg-neutral-800 text-white shadow-lg' : 'bg-black text-white shadow-xl') : (isDark ? 'text-neutral-500 hover:text-white' : 'text-neutral-500 hover:text-black')}`}
                        >
                            <User size={16} /> Profile Info
                        </button>
                        <button
                            onClick={() => setActiveTab('documents')}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'documents' ? (isDark ? 'bg-neutral-800 text-white shadow-lg' : 'bg-black text-white shadow-xl') : (isDark ? 'text-neutral-500 hover:text-white' : 'text-neutral-500 hover:text-black')}`}
                        >
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                <Mail size={16} className="text-[#738D7D]" /> {data.profile.email}
                                            </div>
                                        </div>
                                        {data.profile.phone && (
                                            <div className="space-y-1">
                                                ...
                                                <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                    <Phone size={16} className="text-[#738D7D]" /> {data.profile.phone}
                                                </div>
                                            </div>
                                        )}
                                        <div className="space-y-1">
                                            ...
                                            <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                <MapPin size={16} className="text-[#738D7D]" /> {data.profile.city || 'Remote'}, {data.profile.country || 'Global'}
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div key={i} className="flex gap-4 group">
                                                ...
                                                <div className="pb-8">
                                                    ...
                                                    <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest mt-1 ${isDark ? 'text-neutral-600' : 'text-neutral-400'}`}>
                                                        ...
                                                        <span>—</span>
                                                        <span className={job.isCurrent ? 'text-[#738D7D]' : ''}>{job.isCurrent ? 'Present' : job.endDate}</span>
                                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                {sections.skills.length > 0 && (
                                    <div className={`p-8 rounded-[40px] border ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200 shadow-sm'}`}>
                                        <h3 className={`text-xl font-bold flex items-center gap-2 mb-6 ${isDark ? 'text-white' : 'text-black'}`}>
                                            <BadgeCheck size={20} /> Verified Skills & Evidence
                                        </h3>
                                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {data.sharedDocuments.map((doc) => (
                                    <button
                                        ...
                                        className={`group relative aspect-[4/3] rounded-[32px] border overflow-hidden transition-all duration-300 hover:scale-[1.02] active:scale-95 text-left p-8 flex flex-col justify-between ${activeDocumentType === doc.type ? (isDark ? 'bg-[#738D7D] text-white border-[#738D7D]' : 'bg-[#738D7D] text-white border-[#738D7D] shadow-xl shadow-[#738D7D]/30') : (isDark ? 'bg-neutral-900 border-neutral-800 hover:border-neutral-700' : 'bg-white border-neutral-200 hover:border-[#738D7D]/20 shadow-lg shadow-slate-200/50')}`}
                                    >
                                        <FileText size={40} className={`text-[#738D7D] group-hover:text-white transition-colors ${activeDocumentType === doc.type ? 'text-white' : ''}`} />
                                        <div>
