...
export default function EmployerProfileViewPage() {
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#061D20]">
            <div className="w-12 h-12 border-4 border-[#ED3EB3]/20 border-t-[#ED3EB3] rounded-full animate-spin"></div>
        </div>
    );

    if (!data) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#061D20] space-y-4">
            ...
            <button onClick={() => router.back()} className="text-[#ED3EB3] font-bold hover:underline uppercase text-xs">Go Back</button>
        </div>
    );
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-black text-white' : 'bg-[#F7FCFC] text-black'} overflow-hidden flex`}>
            ...
            <aside className={`w-80 flex-shrink-0 h-screen overflow-y-auto border-r p-6 flex flex-col ${isDark ? 'bg-neutral-900/50 border-neutral-800' : 'bg-white border-neutral-200 shadow-xl z-20'}`}>
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="w-8 h-8 bg-[#ED3EB3] rounded-lg flex items-center justify-center">
                ...
                <div className="text-center space-y-4 mb-2">
                    <div className="relative mx-auto w-32 h-32 rounded-[2rem] overflow-hidden border-4 border-[#ED3EB3]/20 shadow-2xl group cursor-default">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-bold text-[#ED3EB3] uppercase tracking-widest mt-2">{data.profile.role}</p>
                    </div>
                </div>
                ...
                <nav className="flex-1 space-y-2">
                    <button
                        className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-bold transition-all bg-[#ED3EB3] text-white shadow-lg shadow-[#ED3EB3]/20`}
                    >
                        <User size={20} />
                        Profile
                    </button>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                <Mail size={16} className="text-[#ED3EB3]" /> {data.profile.email}
                                            </div>
                                        </div>
                                        {data.profile.phone && (
                                            <div className="space-y-1">
                                                ...
                                                <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                    <Phone size={16} className="text-[#ED3EB3]" /> {data.profile.phone}
                                                </div>
                                            </div>
                                        )}
                                        <div className="space-y-1">
                                            ...
                                            <div className={`flex items-center gap-2 font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                                                <MapPin size={16} className="text-[#ED3EB3]" /> {data.profile.city || 'Remote'}, {data.profile.country || 'Global'}
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div key={i} className="flex gap-4 group">
                                                ...
                                                <div className="pb-8">
                                                    ...
                                                    <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest mt-1 ${isDark ? 'text-neutral-600' : 'text-neutral-400'}`}>
                                                        ...
                                                        <span>—</span>
                                                        <span className={job.isCurrent ? 'text-[#ED3EB3]' : ''}>{job.isCurrent ? 'Present' : job.endDate}</span>
                                                    </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {data.sharedDocuments.map((doc) => (
                                    <button
                                        ...
                                        className={`group relative aspect-[4/3] rounded-[32px] border overflow-hidden transition-all duration-300 hover:scale-[1.02] active:scale-95 text-left p-8 flex flex-col justify-between ${activeDocumentType === doc.type ? (isDark ? 'bg-[#ED3EB3] text-white border-[#ED3EB3]' : 'bg-[#ED3EB3] text-white border-[#ED3EB3] shadow-xl shadow-[#ED3EB3]/30') : (isDark ? 'bg-neutral-900 border-neutral-800 hover:border-neutral-700' : 'bg-white border-neutral-200 hover:border-[#ED3EB3]/20 shadow-lg shadow-slate-200/50')}`}
                                    >
                                        <FileText size={40} className={`text-[#ED3EB3] group-hover:text-white transition-colors ${activeDocumentType === doc.type ? 'text-white' : ''}`} />
                                        <div>
