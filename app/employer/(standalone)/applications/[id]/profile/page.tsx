...
export default function EmployerProfileViewPage() {
    ...
    if (loading) return (
        <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-black' : 'bg-[#F7FCFC]'}`}>
            <div className="w-12 h-12 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin"></div>
        </div>
    );

    if (!data) return (
        <div className={`min-h-screen flex flex-col items-center justify-center space-y-4 ${isDark ? 'bg-black' : 'bg-[#F7FCFC]'}`}>
            ...
            <aside className={`w-80 flex-shrink-0 h-screen overflow-y-auto border-r p-6 flex flex-col ${isDark ? 'bg-neutral-900/50 border-neutral-800' : 'bg-white border-neutral-200 shadow-xl z-20'}`}>
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-[#ED3EB3] rounded-lg flex items-center justify-center">
                ...
                {data.profile.intentMode && data.profile.intentMode !== 'not_looking' && (
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl mb-4 ${data.profile.intentMode === 'actively_looking' ? 'bg-[#ED3EB3]/10 border border-[#ED3EB3]/20' :
                            'bg-blue-500/10 border border-blue-500/20'
                        }`}>
                        <div className="relative flex h-2 w-2">
                            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${data.profile.intentMode === 'actively_looking' ? 'bg-[#35D7DC]' : 'bg-blue-400'
                                }`}></span>
                            <span className={`relative inline-flex rounded-full h-2 w-2 ${data.profile.intentMode === 'actively_looking' ? 'bg-[#ED3EB3]' : 'bg-blue-500'
                                }`}></span>
                        </div>
                        <span className={`text-[10px] font-black uppercase tracking-widest ${data.profile.intentMode === 'actively_looking' ? 'text-[#ED3EB3]' : 'text-blue-500'
                            }`}>
                                ...
                                {verification ? (
                                    <div className="space-y-4">
                                        {verification.checks.map((check, i) => (
                                            <div key={i} className={`flex items-center gap-4 p-4 rounded-2xl border transition-all ${check.status === 'verified' ? 'border-[#ED3EB3]/30 bg-[#ED3EB3]/5 dark:bg-[#ED3EB3]/10' :
                                                    check.status === 'partial' ? 'border-[#ED3EB3]/30 bg-[#ED3EB3]/5 dark:bg-[#ED3EB3]/10' :
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className={`text-xs ${check.status === 'verified' ? 'text-[#ED3EB3] dark:text-[#35D7DC]' :
                                                            check.status === 'partial' ? 'text-[#ED3EB3] dark:text-[#35D7DC]' :
                                                                'text-neutral-400 dark:text-neutral-500'
                                                        }`}>{check.detail}</p>
                                                </div>
                                                <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${check.status === 'verified' ? 'bg-[#ED3EB3]/10 text-[#ED3EB3]' :
                                            ...
                                            <div key={i} className="flex gap-4 group">
                                                <div className="flex flex-col items-center">
                                                    <div className={`w-12 h-12 shrink-0 rounded-xl flex items-center justify-center border z-10 ${isDark ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-neutral-200 shadow-sm'}`}>
                                                        {job.source === 'employer_verified' || job.source === 'application' ? (
                                                            <CheckCircle2 size={24} className="text-[#ED3EB3]" />
                                                        ) : (
                                                ...
                                                <div className="pb-8">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {(job.source === 'employer_verified' || job.source === 'application') && (
                                                            <span className="px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-wider bg-[#ED3EB3]/10 text-[#ED3EB3]">Verified</span>
                                                        )}
                                                    </div>
