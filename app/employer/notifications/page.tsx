...
function ChatContent() {
    ...
    const linkifyText = (text: string, isSenderMessage: boolean = false) => {
        ...
        return parts.map((part, i) => {
            if (urlPattern.test(part)) {
                ...
                const linkClass = isSenderMessage ? 'text-[#9FB5A8] underline hover:text-white break-all font-medium' : (isDark ? 'text-neutral-300 underline hover:text-white break-all' : 'text-neutral-600 underline hover:text-black break-all');
                    ...
                    if (linkMatch) {
                        return (
                            <a key={index} href={linkMatch[2]} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 p-3 my-1 rounded-xl transition-colors border ${isDark ? 'bg-neutral-800 border-neutral-700 hover:bg-neutral-700' : 'bg-white border-neutral-200 hover:bg-neutral-50'}`}>
                                <div className={`p-2 rounded-full ${isDark ? 'bg-neutral-700' : 'bg-[#738D7D]/10'}`}><FileText size={16} className={isDark ? 'text-neutral-300' : 'text-[#738D7D]'} /></div>
    ...
    return (
        <div className="h-full w-full p-1.5 md:p-3 pb-24 md:pb-3">
            <div className={`flex h-full w-full overflow-hidden font-sans rounded-3xl border shadow-2xl ${isDark ? 'bg-[#0D211A]/90 backdrop-blur-2xl border-neutral-800/50 text-white' : 'bg-white/90 backdrop-blur-2xl border-neutral-200/50 text-neutral-800'}`}>
                {/* ── LEFT SIDEBAR ── */}
            <aside className={`md:w-[380px] h-full border-r flex-col backdrop-blur-xl shrink-0 w-full ${isDark ? 'border-white/10 bg-neutral-900/50' : 'border-neutral-200 bg-white'} ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
                ...
                <header className={`p-5 border-b flex items-center justify-between shrink-0 ${isDark ? 'border-white/10 bg-neutral-900/80' : 'border-neutral-200 bg-white'}`}>
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isDark ? 'bg-[#738D7D]/10 text-[#738D7D]' : 'bg-[#738D7D]/10 text-[#738D7D]'}`}><MessageSquare size={20} /></div>
                ...
                <div className={`p-3 shrink-0 ${isDark ? 'bg-neutral-900/80' : 'bg-neutral-50'}`} ref={searchContainerRef}>
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className={`w-full border rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 transition-all ${isDark ? 'bg-neutral-900/50 border-white/10 focus:ring-[#738D7D]/50' : 'bg-white border-neutral-200 focus:ring-[#738D7D]/20'}`}
                        />
                    </div>
                                ...
                                <div className="py-1">
                                    ...
                                    {contactResults.map(result => (
                                        <button key={result.id} onClick={() => startNewConversation(result)} className={`w-full px-4 py-3 flex items-center gap-3 transition-all ${isDark ? 'hover:bg-neutral-800' : 'hover:bg-neutral-50'}`}>
                                            ...
                                            <div className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${isDark ? 'bg-[#738D7D] text-white' : 'bg-[#738D7D] text-white'}`}>Chat</div>
                                        </button>
                                    ))}
                                </div>
                            ) : (
                    ...
                    {activeFilter === 'groups' ? (
                        ...
                        <div className="pb-4">
                            ...
                            {filteredConversations.map((app: any) => {
                                ...
                                return (
                                    <button key={app.id} onClick={() => setActiveConversation(app)} className={`w-full px-3 py-3 flex items-center gap-3 transition-all ${activeConversation?.id === app.id ? (isDark ? 'bg-white/5' : 'bg-[#738D7D]/5 border-[#738D7D]/10') : (isDark ? 'hover:bg-white/5' : 'hover:bg-neutral-100')}`}>
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 relative overflow-hidden ${activeConversation?.id === app.id ? (isDark ? 'bg-[#738D7D] text-white' : 'bg-[#738D7D] text-white') : (isDark ? 'bg-neutral-800 text-neutral-400' : 'bg-neutral-200 text-neutral-500')}`}>
                                        ...
                                        <div className="text-left flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                ...
                                                {activeConversation?.id === app.id && <div className="w-2 h-2 rounded-full bg-[#738D7D]" />}
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className={`text-xs truncate ${isDark ? 'text-[#9FB5A8]' : 'text-[#738D7D]'}`}>{app.job?.title || app.jobTitle}</span>
                        ...
                        <header className={`px-6 py-4 border-b backdrop-blur-md flex items-center justify-between shrink-0 z-10 ${isDark ? 'border-white/10 bg-neutral-900/80' : 'border-neutral-200 bg-white/80'}`}>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className={`text-xs ${isDark ? 'text-[#9FB5A8]' : 'text-[#738D7D]'} font-medium`}>{activeConversation.job?.title || activeConversation.jobTitle}</span>
                                            ...
                                            return (
                                                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`max-w-[85%] md:max-w-[70%] ${isMe ? 'items-end' : 'items-start'}`}>
                                                        <div className={`px-4 py-3 rounded-2xl relative ${isMe ? 'bg-[#738D7D] text-white rounded-br-sm' : (isDark ? 'bg-neutral-800 text-neutral-200 rounded-bl-sm' : 'bg-white border border-neutral-200 text-neutral-800 rounded-bl-sm shadow-sm')}`}>
                                                            {renderMessageContent(msg.content, isMe)}
                                                            {extractFirstUrl(msg.content) && !extractFirstUrl(msg.content)?.includes('public.blob.vercel-storage.com') && (
                                                                <div className="mt-2"><InlineLinkPreview url={extractFirstUrl(msg.content)!} className={isMe ? 'text-[#9FB5A8]' : (isDark ? 'text-neutral-200' : 'text-neutral-800')} /></div>
                                                            )}
                        ...
                        <footer className={`px-4 py-4 border-t shrink-0 relative ${isDark ? 'border-white/10 bg-neutral-900/80' : 'border-neutral-200 bg-white'}`}>
                            ...
                            <div className={`flex items-end gap-3 p-2 rounded-[24px] transition-all border-0 ring-0 outline-none ${isDark ? 'bg-neutral-900' : 'bg-neutral-50'}`}>
                                ...
                                <button
                                    ...
                                    className={`w-10 h-10 flex items-center justify-center rounded-full transition-all shrink-0 mb-0.5 ${isDark ? 'bg-[#738D7D] text-white hover:bg-[#587565] disabled:bg-neutral-800 disabled:text-neutral-600' : 'bg-[#738D7D] text-white hover:bg-[#587565] disabled:bg-neutral-200 disabled:text-neutral-400'}`}
                                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className={`w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] ${isDark ? 'bg-neutral-900 border border-neutral-800' : 'bg-white border border-neutral-200'}`}>
                        ...
                        <div className={`p-4 border-t flex items-center justify-between ${isDark ? 'border-neutral-800 bg-neutral-900/50' : 'border-neutral-200 bg-neutral-50'}`}>
                            <button className={`flex items-center gap-1.5 text-xs font-bold px-4 py-2 rounded-xl transition-all ${isDark ? 'bg-[#738D7D] text-white hover:bg-[#587565]' : 'bg-[#738D7D] text-white hover:bg-[#587565]'}`}><UserPlus size={14} /> Add Member</button>
                        </div>
                    </div>
                </div>
            )}
...
export default function EmployerChatPage() {
    return (
        <React.Suspense fallback={<div className="flex items-center justify-center h-screen"><div className="animate-spin w-8 h-8 border-2 border-t-transparent border-[#738D7D] rounded-full" /></div>}>
            <ChatContent />
        </React.Suspense>
    );
}
