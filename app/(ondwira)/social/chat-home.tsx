...
export default function ChatHome() {
  ...
  useEffect(() => {
    const search = contactQuery.trim().replace(/^@+/, '').toLowerCase();
    if (!picker || search.length < 2 || !/^[a-z0-9_]+$/.test(search)) {
      return;
    }
    const controller = new AbortController();
    const timer = window.setTimeout(() => {
      setDiscovering(true);
      fetch(`/api/social/discovery?query=${encodeURIComponent(search)}`, { signal: controller.signal, cache: 'no-store' })
  ...
  const matchingContacts = useMemo(() => {
    const search = contactQuery.trim().replace(/^@+/, '').toLowerCase();
    const approved = contacts.filter(contact => !search || contact.name.toLowerCase().includes(search) || contact.username?.includes(search));
    const discoveryMatches = search.length >= 2 ? discovered : [];
    const merged = new Map<string, Contact>();
    [...selectedContacts, ...approved, ...discoveryMatches].forEach(contact => merged.set(contact.id, contact));
    return [...merged.values()].filter(contact => !search || contact.name.toLowerCase().includes(search) || contact.username?.includes(search));
  ...
  return <div className="flex h-[calc(100dvh-4rem)] min-h-0 bg-transparent">
    ...
    <section className={`${selected ? 'flex' : 'hidden md:flex'} min-h-0 min-w-0 flex-1`}>{active ? <ConversationPanel conversationId={active.conversation_id} context="social" title={title} subtitle={active.conversations.kind === 'group' ? 'Social group' : 'Private conversation'} onBack={() => setSelected(null)} /> : <div className="m-auto px-6 text-center"><h2 className="font-editorial text-3xl">Your conversations, without the noise.</h2><p className="mt-3 text-sm text-[var(--text-secondary)]">A username is enough to find someone. Your private phone stays hidden.</p><button onClick={() => setPicker(true)} className="mt-5 rounded-2xl bg-[var(--accent-primary)] px-5 py-3 text-sm font-bold text-[var(--text-inverse)]">New chat</button></div>}</section>
    ...
    {picker && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/60 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Start a conversation">
      <div className="w-full max-w-md rounded-[28px] border border-[var(--border-secondary)] bg-[var(--surface-raised)] p-5 shadow-2xl">
        ...
        <label className="mt-4 flex items-center gap-3 rounded-2xl border border-transparent bg-[var(--surface-muted)] px-4 py-3 text-[var(--text-secondary)] focus-within:border-[var(--accent-primary)]"><AtSign size={18} className="text-[var(--accent-primary)]" /><input value={contactQuery} onChange={event => setContactQuery(event.target.value.replace(/^@+/, '').toLowerCase().replace(/[^a-z0-9_]/g, ''))} className="w-full bg-transparent text-sm text-[var(--text-primary)] outline-none" placeholder="Search unique username" />{discovering && <LoaderCircle size={17} className="animate-spin" />}</label>
        <p className="mt-2 px-1 text-[10px] leading-4 text-[var(--text-muted)]">Type at least two characters. Email and phone numbers are never shown.</p>
        <div className="ondwira-scrollbar mt-3 max-h-72 overflow-y-auto">
          {!matchingContacts.length && !discovering && <p className="py-8 text-center text-sm text-[var(--text-secondary)]">{contactQuery.length >= 2 ? 'No matching username found.' : contacts.length ? 'Your approved contacts are ready, or search any @username.' : 'Search an @username to begin.'}</p>}
