    ...
    <div className="relative min-h-dvh overflow-hidden bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--border-secondary)] bg-[color:var(--surface-raised)]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-4 px-4 sm:px-6">
          ...
          <div className="mx-auto flex rounded-full bg-[var(--surface-muted)] p-1 text-sm font-bold" aria-label="Choose Ondwira mode">
            <button onClick={() => changeMode('social')} className={`rounded-full px-5 py-2 transition ${mode === 'social' ? 'bg-[var(--surface-raised)] text-[var(--accent-primary)] shadow-sm' : 'text-[var(--text-secondary)]'}`}>Social</button>
            <button onClick={() => changeMode('work')} className={`rounded-full px-5 py-2 transition ${mode === 'work' ? 'bg-[var(--accent-primary)] text-[var(--text-inverse)] shadow-sm' : 'text-[var(--text-secondary)]'}`}>Work</button>
          </div>
      ...
      <div className="relative z-10 mx-auto flex max-w-[1600px]">
        <aside className="fixed inset-x-0 bottom-0 z-40 flex h-16 items-center justify-around border-t border-[var(--border-secondary)] bg-[var(--surface-raised)] px-2 md:sticky md:top-16 md:h-[calc(100dvh-4rem)] md:w-60 md:shrink-0 md:flex-col md:items-stretch md:justify-start md:border-r md:border-t-0 md:p-4">
          ...
          <button onClick={() => router.push('/settings')} className={`flex flex-col items-center gap-1 rounded-xl px-3 py-2 text-[10px] font-bold md:flex-row md:gap-3 md:text-sm ${pathname === '/settings' ? 'bg-[var(--accent-primary)] text-[var(--text-inverse)]' : 'text-[var(--text-secondary)] hover:bg-[var(--surface-muted)]'}`}><Settings size={19} /><span>Settings</span></button>
