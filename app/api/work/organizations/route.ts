...
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin.schema('profcaria').from('organization_members')
...
export async function POST(request: Request) {
  ...
  const { error: membershipError } = await supabaseAdmin.schema('profcaria').from('organization_members').insert({ organization_id: organization.id, user_id: session.uid, account_type: session.schema, role: 'owner', status: 'active', joined_at: new Date().toISOString() });
  if (membershipError) {
    await supabaseAdmin.schema('profcaria').from('organizations').delete().eq('id', organization.id);
    return NextResponse.json({ error: 'Unable to create the organisation membership' }, { status: 500 });
  }

  const { data: conversation, error: conversationError } = await supabaseAdmin.schema('profcaria').from('conversations')
  ...
  const { error: conversationMemberError } = await supabaseAdmin.schema('profcaria').from('conversation_members').insert({ conversation_id: conversation.id, user_id: session.uid, account_type: session.schema, role: 'owner', membership_status: 'accepted' });
  ...
  const { data: group, error: groupError } = await supabaseAdmin.schema('profcaria').from('work_groups').insert({ organization_id: organization.id, conversation_id: conversation.id, name: `${name} company`, group_type: 'company', auto_membership: true, created_by: session.uid }).select('id, name, group_type, auto_membership, conversation_id, created_at').single();
  if (groupError || !group) {
    await supabaseAdmin.schema('profcaria').from('conversations').delete().eq('id', conversation.id);
    await supabaseAdmin.schema('profcaria').from('organizations').delete().eq('id', organization.id);
    return NextResponse.json({ error: 'The organisation could not complete its automatic company group.' }, { status: 500 });
  }
  const { error: groupMemberError } = await supabaseAdmin.schema('profcaria').from('work_group_members').insert({ group_id: group.id, user_id: session.uid, role: 'owner', membership_source: 'organization' });
  if (groupMemberError) {
    await supabaseAdmin.schema('profcaria').from('conversations').delete().eq('id', conversation.id);
    await supabaseAdmin.schema('profcaria').from('organizations').delete().eq('id', organization.id);
