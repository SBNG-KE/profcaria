...
async function loadApplication(id: string) {
  const { data } = await supabaseAdmin.schema('profcaria').from('applications')
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    supabaseAdmin.schema('profcaria').from('application_answers')
      .select('enc_answer, passed_knockout, awarded_score, job_questions(id, enc_prompt, question_type)')
      .eq('application_id', id),
    supabaseAdmin.schema('profcaria').from('application_documents')
    ...
    supabaseAdmin.schema('profcaria').from('application_stage_events').select('*')
      .eq('application_id', id).order('created_at', { ascending: false }),
    supabaseAdmin.schema('profcaria').from('recruitment_interviews').select('*')
      .eq('application_id', id).order('starts_at', { ascending: false }),
    supabaseAdmin.schema('profcaria').from('job_offers').select('*, contracts(status)')
      .eq('application_id', id).order('version', { ascending: false }),
    supabaseAdmin.schema('profcaria').from('application_ai_evaluations').select('*')
      .eq('application_id', id).order('created_at', { ascending: false }).limit(5),
    resolveProfcariaAccounts([application.applicant_id]),
  ]);
  ...
  return NextResponse.json({
    ...
    application: {
      ...
      candidate: { id: application.applicant_id, name: people.get(application.applicant_id)?.name || snapshot.name || 'Profcaria member' },
      snapshot,
      job: {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  async function changeStatus(nextStatus: string, scope: 'applicant' | 'organization', reasonCode?: string, note?: string) {
    ...
    const { error } = await supabaseAdmin.schema('profcaria').from('applications').update(update).eq('id', id);
  ...
  if (action === 'withdraw') {
    ...
    await supabaseAdmin.schema('profcaria').from('application_documents').update({ access_revoked_at: now }).eq('application_id', id);
  ...
  if (action === 'interview_response') {
    ...
    const { data: interview } = await supabaseAdmin.schema('profcaria').from('recruitment_interviews')
    ...
    if (interview.work_meeting_id) {
      await supabaseAdmin.schema('profcaria').from('work_meeting_participants').update({
  ...
  if (action === 'offer_response') {
    ...
    const { data: offer } = await supabaseAdmin.schema('profcaria').from('job_offers')
  ...
  if (action === 'resign') {
    ...
    await supabaseAdmin.schema('profcaria').from('employment_records').update({
      status: 'notice', ended_at: lastDay, end_reason: 'resignation', last_event_at: now,
    }).eq('id', record.id);
    await supabaseAdmin.schema('profcaria').from('employment_events').insert({
  ...
  if (action === 'review') {
    ...
    await supabaseAdmin.schema('profcaria').from('application_reviews').upsert({
  ...
  if (action === 'schedule_interview') {
    ...
    const provider = ['profcaria', 'google_meet', 'zoom', 'teams', 'jitsi', 'custom', 'in_person', 'phone'].includes(input.provider)
      ? input.provider : 'profcaria';
    ...
    const { data: meeting, error: meetingError } = await supabaseAdmin.schema('profcaria').from('work_meetings').insert({
      ...
      native_room_ready: provider === 'profcaria' ? false : false,
    }).select('id').single();
    if (meetingError || !meeting) return NextResponse.json({ error: 'Interview meeting could not be created.' }, { status: 500 });
    await supabaseAdmin.schema('profcaria').from('work_meeting_participants').insert([
    ...
    const { data: interview, error } = await supabaseAdmin.schema('profcaria').from('recruitment_interviews').insert({
  ...
  if (action === 'send_offer') {
    ...
    const { data: previous } = await supabaseAdmin.schema('profcaria').from('job_offers')
      .select('version').eq('application_id', id).order('version', { ascending: false }).limit(1);
    const version = Number(previous?.[0]?.version || 0) + 1;
    const { data: document } = await supabaseAdmin.schema('profcaria').from('documents').insert({
    ...
    const { data: contract } = await supabaseAdmin.schema('profcaria').from('contracts').insert({
    ...
    const { data: offer, error } = await supabaseAdmin.schema('profcaria').from('job_offers').insert({
  ...
  if (action === 'hire') {
    ...
    const { data: identity } = await supabaseAdmin.schema('profcaria').from('account_identities')
    ...
    const { data: employment, error } = await supabaseAdmin.schema('profcaria').from('employment_records').upsert({
      ...
      verification_status: 'verified', verification_method: 'profcaria_lifecycle',
  ...
  if (action === 'end_employment') {
    ...
    const { data: employment } = await supabaseAdmin.schema('profcaria').from('employment_records')
