...
import { addAutomaticOrganizationAccess, removeOrganizationAccess, resolveOrganizationAccountType } from '@/lib/profcaria-organizations';
...
export async function GET() {
  ...
  await supabaseAdmin.schema('profcaria').from('organization_invitations').update({ status: 'expired' })
    .eq('invitee_account_id', session.uid).eq('status', 'pending').lte('expires_at', now);
  const { data, error } = await supabaseAdmin.schema('profcaria').from('organization_invitations')
  ...
  const inviters = await resolveProfcariaAccounts((data ?? []).map((invitation: { invited_by: string }) => invitation.invited_by));
...
export async function PATCH(request: Request) {
  ...
  const { data: invitation, error } = await supabaseAdmin.schema('profcaria').from('organization_invitations')
  ...
  if (new Date(invitation.expires_at).getTime() <= Date.now()) {
    await supabaseAdmin.schema('profcaria').from('organization_invitations').update({ status: 'expired' }).eq('id', invitation.id);
    return NextResponse.json({ error: 'This invitation has expired.' }, { status: 410 });
  }
  if (input.action === 'decline') {
    const { error: declineError } = await supabaseAdmin.schema('profcaria').from('organization_invitations').update({ status: 'declined' }).eq('id', invitation.id).eq('status', 'pending');
  ...
  try {
    ...
    await supabaseAdmin.schema('profcaria').from('organization_members').update({ status: 'invited', ended_at: now }).eq('organization_id', invitation.organization_id).eq('user_id', session.uid);
    console.error('[PROFCARIA] invitation group sync failed', syncError);
    return NextResponse.json({ error: 'Work access could not be synchronized. Please try accepting again.' }, { status: 500 });
  }
  const { error: acceptError } = await supabaseAdmin.schema('profcaria').from('organization_invitations').update({
    status: 'accepted', accepted_by: session.uid, accepted_at: now,
  }).eq('id', invitation.id).eq('status', 'pending');
  if (acceptError) {
    await supabaseAdmin.schema('profcaria').from('organization_members')
