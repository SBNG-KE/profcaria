...
export async function GET(request: Request) {
  ...
  const { data: viewer } = await supabaseAdmin.schema('profcaria').from('organization_members').select('status').eq('organization_id', organizationId).eq('user_id', session.uid).maybeSingle(); if (viewer?.status !== 'active') return NextResponse.json({ error: 'Not found' }, { status: 404 });
  const { data: rows, error } = await supabaseAdmin.schema('profcaria').from('organization_members').select('user_id, account_type, role').eq('organization_id', organizationId).eq('status', 'active'); if (error) return NextResponse.json({ error: 'Unable to load people' }, { status: 500 });
