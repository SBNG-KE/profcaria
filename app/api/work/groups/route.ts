...
async function membership(organizationId: string, userId: string) {
  const { data } = await supabaseAdmin.schema('profcaria').from('organization_members').select('role, status').eq('organization_id', organizationId).eq('user_id', userId).maybeSingle();
  return data;
}
...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.schema('profcaria').from('work_groups').select('id, name, group_type, auto_membership, conversation_id, created_at').eq('organization_id', organizationId).is('archived_at', null).order('created_at');
...
export async function POST(request: Request) {
  ...
  const { data: allowed } = await supabaseAdmin.schema('profcaria').from('organization_members').select('user_id, account_type').eq('organization_id', input.organizationId).eq('status', 'active').in('user_id', requested);
  ...
  const { data: conversation, error: conversationError } = await supabaseAdmin.schema('profcaria').from('conversations').insert({ kind: 'group', context: 'work', title: name, created_by: session.uid }).select('id').single();
  if (conversationError || !conversation) return NextResponse.json({ error: 'Unable to create group conversation' }, { status: 500 });
  const { data: group, error: groupError } = await supabaseAdmin.schema('profcaria').from('work_groups').insert({ organization_id: input.organizationId, conversation_id: conversation.id, name, group_type: groupType, auto_membership: false, created_by: session.uid }).select('id, name, group_type, auto_membership, conversation_id, created_at').single();
  if (groupError || !group) {
  ...
  const { error: groupMemberError } = await supabaseAdmin.schema('profcaria').from('work_group_members').insert(memberIds.map((userId: string) => ({ group_id: group.id, user_id: userId, role: userId === session.uid ? 'owner' : 'member', membership_source: 'manual' })));
  const { error: conversationMemberError } = groupMemberError ? { error: groupMemberError } : await supabaseAdmin.schema('profcaria').from('conversation_members').insert(memberIds.map((userId: string) => ({ conversation_id: conversation.id, user_id: userId, account_type: accountTypes.get(userId) || session.schema, role: userId === session.uid ? 'owner' : 'member', membership_status: 'accepted' })));
  if (groupMemberError || conversationMemberError) {
    await supabaseAdmin.schema('profcaria').from('work_groups').delete().eq('id', group.id);
    await supabaseAdmin.schema('profcaria').from('conversations').delete().eq('id', conversation.id);
