...
export async function POST(request: Request) {
  const session = await getOndwiraSession();
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const input = await request.json().catch(() => null) as { organizationId?: string; name?: string; memberIds?: string[] } | null;
  ...
  const { data: group, error: groupError } = await supabaseAdmin.schema('ondwira').from('work_groups').insert({ organization_id: input.organizationId, conversation_id: conversation.id, name, group_type: 'custom', auto_membership: false, created_by: session.uid }).select('id, name, group_type, auto_membership, conversation_id, created_at').single();
  if (groupError || !group) return NextResponse.json({ error: 'Unable to create group' }, { status: 500 });
  await supabaseAdmin.schema('ondwira').from('work_group_members').insert(memberIds.map((userId: string) => ({ group_id: group.id, user_id: userId, role: userId === session.uid ? 'owner' : 'member', membership_source: 'manual' })));
  await supabaseAdmin.schema('ondwira').from('conversation_members').insert(memberIds.map((userId: string) => ({ conversation_id: conversation.id, user_id: userId, account_type: accountTypes.get(userId) || session.schema, role: userId === session.uid ? 'owner' : 'member', membership_status: 'accepted' })));
