...
import { getOrganizationMembership, ORGANIZATION_MANAGER_ROLES } from '@/lib/profcaria-organizations';
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: group, error } = await supabaseAdmin.schema('profcaria').from('work_groups')
  ...
  if (input.action === 'rename') {
    ...
    const { error: updateError } = await supabaseAdmin.schema('profcaria').from('work_groups').update({ name }).eq('id', id);
    if (updateError) return NextResponse.json({ error: 'The group could not be renamed.' }, { status: 500 });
    if (group.conversation_id) await supabaseAdmin.schema('profcaria').from('conversations').update({ title: name, updated_at: new Date().toISOString() }).eq('id', group.conversation_id);
  ...
  if (input.action === 'archive') {
    ...
    const { error: archiveError } = await supabaseAdmin.schema('profcaria').from('work_groups').update({ archived_at: now }).eq('id', id);
    if (archiveError) return NextResponse.json({ error: 'The group could not be archived.' }, { status: 500 });
    await supabaseAdmin.schema('profcaria').from('work_group_members').update({ removed_at: now }).eq('group_id', id).is('removed_at', null);
    if (group.conversation_id) await supabaseAdmin.schema('profcaria').from('conversation_members').update({ membership_status: 'removed' }).eq('conversation_id', group.conversation_id);
  ...
  if (input.action === 'add_member') {
    const now = new Date().toISOString();
    const { data: existing } = await supabaseAdmin.schema('profcaria').from('work_group_members').select('role').eq('group_id', id).eq('user_id', input.userId).maybeSingle();
    const { error: memberError } = await supabaseAdmin.schema('profcaria').from('work_group_members').upsert({
      group_id: id,
    ...
    if (group.conversation_id) {
      const { error: conversationError } = await supabaseAdmin.schema('profcaria').from('conversation_members').upsert({
  ...
  const { data: groupMembership } = await supabaseAdmin.schema('profcaria').from('work_group_members').select('role').eq('group_id', id).eq('user_id', input.userId).maybeSingle();
  if (groupMembership?.role === 'owner') return NextResponse.json({ error: 'The group owner cannot be removed.' }, { status: 409 });
  const now = new Date().toISOString();
  const { error: removeError } = await supabaseAdmin.schema('profcaria').from('work_group_members').update({ removed_at: now }).eq('group_id', id).eq('user_id', input.userId).is('removed_at', null);
  if (removeError) return NextResponse.json({ error: 'The person could not be removed from this group.' }, { status: 500 });
  if (group.conversation_id) await supabaseAdmin.schema('profcaria').from('conversation_members').update({ membership_status: 'removed' }).eq('conversation_id', group.conversation_id).eq('user_id', input.userId);
