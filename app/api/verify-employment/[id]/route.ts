...
async function loadVerification(id: string, token: string) {
  ...
  const { data } = await supabaseAdmin.schema('profcaria').from('employment_verification_requests')
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const people = await resolveProfcariaAccounts([verification.employment_records.user_id]);
  return NextResponse.json({
    verification: {
      id,
      workerName: people.get(verification.employment_records.user_id)?.name || 'Profcaria member',
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await Promise.all([
    supabaseAdmin.schema('profcaria').from('employment_verification_requests').update({
