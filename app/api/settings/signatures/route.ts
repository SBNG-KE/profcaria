...
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin.schema('profcaria').from('signatures').select('id, label, source, aspect_ratio, is_default, created_at, last_used_at').eq('owner_id', session.uid).is('revoked_at', null).order('created_at', { ascending: false });
...
export async function POST(request: Request) {
  ...
  if (input?.isDefault) await supabaseAdmin.schema('profcaria').from('signatures').update({ is_default: false }).eq('owner_id', session.uid).is('revoked_at', null);
  const { data, error } = await supabaseAdmin.schema('profcaria').from('signatures').insert({ owner_id: session.uid, label: input?.label?.trim().slice(0, 80) || 'My signature', enc_vector_data: encryptData(JSON.stringify(normalized)), source, aspect_ratio: width / height, is_default: Boolean(input?.isDefault) }).select('id, label, source, aspect_ratio, is_default, created_at').single();
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabaseAdmin.schema('profcaria').from('signatures').update({ revoked_at: new Date().toISOString(), is_default: false }).eq('id', id).eq('owner_id', session.uid);
