...
export async function POST(request: Request) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    const blob = await put(`profcaria/documents/${session.uid}/${sanitizeFilename(file.name)}`, file, { access: 'public', addRandomSuffix: true });
  ...
  const { data, error } = await supabaseAdmin.schema('profcaria').from('documents').insert(record).select('id, document_kind, source_type, created_at').single();
...
export async function DELETE(request: Request) {
  ...
  const { data } = await supabaseAdmin.schema('profcaria').from('documents').select('enc_file_url').eq('id', id).eq('owner_id', session.uid).maybeSingle();
  if (!data) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  const fileUrl = decryptData(data.enc_file_url); if (fileUrl) await del(fileUrl).catch(error => console.error('[PROFCARIA] Blob deletion failed', error));
  await supabaseAdmin.schema('profcaria').from('documents').update({ archived_at: new Date().toISOString() }).eq('id', id).eq('owner_id', session.uid);
