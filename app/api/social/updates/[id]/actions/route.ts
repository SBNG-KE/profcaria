...
async function canView(updateId: string, userId: string, contactIds: string[]) {
  const { data: update } = await supabaseAdmin.schema('profcaria').from('social_updates')
  ...
  const { data: audience } = await supabaseAdmin.schema('profcaria').from('social_update_audience').select('update_id').eq('update_id', updateId).eq('user_id', userId).maybeSingle();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (input?.action === 'view') {
    ...
    await supabaseAdmin.schema('profcaria').from('social_update_views').upsert({
  ...
  if (input?.action === 'react') {
    ...
    if (!emoji) {
      await supabaseAdmin.schema('profcaria').from('social_update_reactions').delete().eq('update_id', id).eq('user_id', session.uid);
      return NextResponse.json({ reaction: null });
    }
    await supabaseAdmin.schema('profcaria').from('social_update_reactions').upsert({ update_id: id, user_id: session.uid, emoji }, { onConflict: 'update_id,user_id' });
  ...
  if (input?.action === 'reply') {
    ...
    const { error } = await supabaseAdmin.schema('profcaria').from('social_update_replies').insert({ update_id: id, author_id: session.uid, enc_body: encryptData(body) });
