...
import { getConversationAccess } from '@/lib/profcaria-chat';
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: members } = await supabaseAdmin.schema('profcaria').from('conversation_members').select('user_id, role').eq('conversation_id', id).eq('membership_status', 'accepted');
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (input?.action === 'mute') {
    ...
    await supabaseAdmin.schema('profcaria').from('conversation_members').update({ muted_until: mutedUntil }).eq('conversation_id', id).eq('user_id', session.uid);
  ...
  if (input?.action === 'disappearing') {
    ...
    await supabaseAdmin.schema('profcaria').from('conversations').update({ disappearing_seconds: input.seconds }).eq('id', id);
    return NextResponse.json({ seconds: input.seconds });
  }

  if (input?.action === 'view_once_default') {
    await supabaseAdmin.schema('profcaria').from('conversations').update({ view_once_default: Boolean(input.enabled) }).eq('id', id);
  ...
  if (input?.action === 'report') {
    ...
    await supabaseAdmin.schema('profcaria').from('conversation_reports').insert({ conversation_id: id, message_id: input.messageId || null, reporter_id: session.uid, encrypted_reason: encryptData(reason) });
  ...
  if (input?.action === 'block') {
    ...
    const { data: targetMember } = await supabaseAdmin.schema('profcaria').from('conversation_members').select('user_id').eq('conversation_id', id).eq('user_id', input.accountId).eq('membership_status', 'accepted').maybeSingle();
    if (!targetMember) return NextResponse.json({ error: 'That account is not part of this conversation' }, { status: 400 });
    await supabaseAdmin.schema('profcaria').from('blocked_accounts').upsert({ blocker_id: session.uid, blocked_id: input.accountId }, { onConflict: 'blocker_id,blocked_id' });
