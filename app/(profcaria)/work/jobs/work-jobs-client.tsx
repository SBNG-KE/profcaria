...
function JobComposer({ organizationName, form, setForm, questions, setQuestions, busy, close, create }: {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/65 p-3 backdrop-blur-sm"><div className="mx-auto my-4 grid w-full max-w-6xl overflow-hidden rounded-[34px] border border-white/10 bg-[var(--surface-raised)] shadow-2xl lg:grid-cols-[0.7fr_1.3fr]">
    <aside className="bg-[var(--accent-primary)] p-7 text-[var(--text-inverse)] sm:p-10"><p className="text-[10px] font-black uppercase tracking-[0.25em] opacity-60">{organizationName}</p><h2 className="font-editorial mt-5 text-5xl leading-none">One role. One complete trail.</h2><p className="mt-5 text-sm leading-7 opacity-70">Profcaria records the publication, every tracked share, consented evidence, human reviews, AI assistance, interviews, offers, signatures and employment events.</p><div className="mt-10 space-y-3 text-xs font-bold"><p>• AI never uses protected personal attributes.</p><p>• AI can hold or shortlist for review, never silently reject.</p><p>• Candidates apply with documents already in Settings.</p></div></aside>
