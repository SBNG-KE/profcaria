...
export default function SettingsClient() {
  ...
  useEffect(() => {
    const sync = () => setFindWork(localStorage.getItem('profcaria_find_work') === 'true');
    const frame = window.requestAnimationFrame(sync);
    window.addEventListener('profcaria:preferences', sync);
    return () => {
      window.cancelAnimationFrame(frame);
      window.removeEventListener('profcaria:preferences', sync);
    };
  }, []);

  function updateFindWork(enabled: boolean) {
    setFindWork(enabled);
    localStorage.setItem('profcaria_find_work', String(enabled));
    window.dispatchEvent(new Event('profcaria:preferences'));
  }
  ...
  async function logOut() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Profcaria could not close this session.');
      ...
      setLogoutError(error instanceof Error ? error.message : 'Profcaria could not close this session.');
      setLoggingOut(false);
    }
  }
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-7 sm:py-9 lg:px-10 lg:py-12">
      <div className="overflow-hidden rounded-[28px] border border-[var(--border-secondary)] bg-[var(--surface-raised)]/90 shadow-[0_24px_80px_rgba(0,0,0,0.08)] backdrop-blur-sm sm:rounded-[34px]">
        <div className="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(19rem,0.75fr)]">
          <header className="relative border-b border-[var(--border-secondary)] p-6 sm:p-9 lg:border-b-0 lg:border-r lg:p-12">
            ...
            <h1 className="mt-5 max-w-2xl font-editorial text-4xl leading-[0.96] tracking-[-0.04em] sm:text-6xl">Your life in Profcaria, arranged around you.</h1>
      ...
      <div className="mt-5 rounded-[26px] border border-[var(--border-secondary)] bg-[var(--surface-raised)]/88 p-5 backdrop-blur-sm sm:p-8">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[var(--accent-primary)]">Typography cabinet</p><h2 className="mt-2 font-editorial text-3xl">Choose how Profcaria reads</h2></div><span className="text-xs text-[var(--text-muted)]">Saved to your account</span></div>
      ...
      <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[var(--accent-primary)]">Private rooms</p><h2 className="mt-2 font-editorial text-3xl sm:text-4xl">Everything has its proper place.</h2></div><p className="max-w-sm text-xs leading-5 text-[var(--text-muted)]">Available rooms open now. Areas still being moved into Profcaria are clearly marked.</p></div>
      ...
      <div className="mt-5 flex flex-col gap-5 rounded-[24px] border border-[var(--border-secondary)] bg-[var(--surface-raised)]/90 p-5 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div>
            ...
            <h2 className="mt-1 font-editorial text-2xl">Leave Profcaria safely</h2>
