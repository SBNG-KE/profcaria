...
const backgrounds: Record<BackgroundStyle, { background: string; color: string; label: string }> = {
  parchment: { background: 'linear-gradient(145deg, #DDE7DF, #F49BD5)', color: '#153027', label: 'Parchment' },
  terracotta: { background: 'linear-gradient(145deg, #738D7D, #143D33)', color: '#FFF0FB', label: 'Terracotta' },
  ink: { background: 'linear-gradient(145deg, #081812, #081812)', color: '#FFF0FB', label: 'Midnight ink' },
  olive: { background: 'linear-gradient(145deg, #3D6254, #153027)', color: '#FFF0FB', label: 'Old olive' },
  gold: { background: 'linear-gradient(145deg, #738D7D, #143D33)', color: '#081812', label: 'Muted gold' },
  rose: { background: 'linear-gradient(145deg, #738D7D, #143D33)', color: '#FFF0FB', label: 'Dusty rose' },
};
  ...
  return <section className="relative min-h-full overflow-hidden px-4 pb-28 pt-5 sm:px-7 sm:pb-10 sm:pt-8">
    ...
    <div className="relative mx-auto max-w-6xl">
      ...
      <section className="mt-7">
        ...
        <div className="profcaria-scrollbar flex snap-x gap-3 overflow-x-auto pb-3">
...
function MomentComposer({ contacts, onClose, onPublished, onNotice }: { contacts: Contact[]; onClose: () => void; onPublished: () => void; onNotice: (message: string) => void }) {
  ...
  return <div className="profcaria-scrollbar fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-black/65 p-3 backdrop-blur-md sm:p-6" role="dialog" aria-modal="true" aria-label="Create a Social update">
    <div className="my-auto grid w-full max-w-5xl overflow-hidden rounded-[34px] border border-white/10 bg-[var(--surface-raised)] shadow-2xl lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="profcaria-scrollbar max-h-[85dvh] overflow-y-auto p-5 sm:p-8">
...
function MomentViewer({ update, index, total, onClose, onPrevious, onNext, onRefresh, onDelete }: { update: SocialUpdate; index: number; total: number; onClose: () => void; onPrevious: () => void; onNext: () => void; onRefresh: () => void; onDelete: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] grid bg-[#081812] text-white lg:grid-cols-[1fr_350px]" role="dialog" aria-modal="true" aria-label={`${update.authorName}'s update`}>
    ...
    <aside className="profcaria-scrollbar min-h-0 overflow-y-auto border-l border-white/10 bg-[#081812] p-5 pt-7">
