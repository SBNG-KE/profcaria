...
export default function ChatHome() {
  ...
  const title = active?.displayTitle || active?.conversations.title || (active?.conversations.kind === 'group' ? 'Untitled group' : 'Profcaria member');
  ...
  return <div className="flex h-[calc(100dvh-8.5rem)] min-h-0 bg-transparent md:h-[calc(100dvh-4rem)]">
    <section className={`${selected ? 'hidden md:flex' : 'flex'} w-full shrink-0 flex-col border-r border-[var(--border-secondary)] bg-[var(--surface-raised)]/88 md:w-[360px]`}>
      ...
      <div className="profcaria-scrollbar min-h-0 flex-1 overflow-y-auto px-3 pb-4">
        ...
        {filtered.map(chat => { const chatTitle = chat.displayTitle || chat.conversations.title || (chat.conversations.kind === 'group' ? 'Untitled group' : 'Profcaria member'); return <button key={chat.conversation_id} onClick={() => setSelected(chat.conversation_id)} className={`mb-1 flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${selected === chat.conversation_id ? 'bg-[var(--accent-soft)]' : 'hover:bg-[var(--surface-muted)]'}`}><span className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-[var(--surface-muted)] font-black text-[var(--accent-primary)]">{chat.avatarUrl ? <img src={chat.avatarUrl} alt="" className="h-full w-full object-cover" /> : chatTitle[0]}</span><span className="min-w-0"><span className="block truncate text-sm font-black">{chatTitle}</span><span className="block truncate text-xs text-[var(--text-secondary)]">{chat.displaySubtitle || (chat.conversations.kind === 'group' ? 'Social group' : 'Direct conversation')}</span></span></button>; })}
      </div>
    </section>
    ...
    {picker && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/60 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Start a conversation">
      <div className="w-full max-w-md rounded-[28px] border border-[var(--border-secondary)] bg-[var(--surface-raised)] p-5 shadow-2xl">
        <div className="flex justify-between"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--text-muted)]">New chat</p><h2 className="mt-1 text-xl font-black">{pickerMode === 'direct' ? 'Find an Profcaria account' : 'Create a group'}</h2></div><button onClick={closePicker} aria-label="Close"><X /></button></div>
