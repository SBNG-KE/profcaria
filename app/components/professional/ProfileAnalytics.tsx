...
const ProfileAnalytics = ({ isDark }: AnalyticsProps) => {
    ...
    const [stats, setStats] = useState<any>({ followers: 0, likes: 0, comments: 0, reposts: 0, views: 0, viewDates: [], recentFollowers: [] });
                        ...
                        <AreaChart data={viewData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                            <defs>
                                <linearGradient id="colorViews" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="5%" stopColor={isDark ? '#00A7B5' : '#006B76'} stopOpacity={0.3} />
                                    <stop offset="95%" stopColor={isDark ? '#00A7B5' : '#006B76'} stopOpacity={0} />
                                </linearGradient>
                            </defs>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? '#10282B' : '#E8F1F2'} />
                            <XAxis
                                dataKey="name"
                                axisLine={false}
                                tickLine={false}
                                tick={{ fill: isDark ? '#3B575B' : '#AFC9CD', fontSize: 10, fontWeight: 600 }}
                                dy={10}
                            />
                            <YAxis
                                axisLine={false}
                            ...
                            />
                            <Tooltip
                                cursor={{ stroke: isDark ? '#3B575B' : '#E8F1F2', strokeWidth: 1, strokeDasharray: '3 3' }}
                                contentStyle={{
                                    backgroundColor: isDark ? '#061416' : '#F7FCFC',
                                    borderColor: isDark ? '#10282B' : '#E8F1F2',
                                    borderRadius: '12px',
                                    fontSize: '12px',
                                    fontWeight: 'bold',
                                    color: isDark ? '#F7FCFC' : '#061416',
                                    boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)'
                                }}
                                itemStyle={{
                                    color: isDark ? '#00A7B5' : '#006B76',
                                    fontWeight: '900'
                                }}
                            />
                            <Area
                                type="monotone"
                                dataKey="views"
                                stroke={isDark ? '#00A7B5' : '#006B76'}
                                strokeWidth={2}
                                fillOpacity={1}
                                fill="url(#colorViews)"
                                activeDot={{ r: 5, fill: isDark ? '#00A7B5' : '#006B76', stroke: isDark ? '#061416' : '#F7FCFC', strokeWidth: 2 }}
                            />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
