            ...
            <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="flex flex-col gap-6">
                        {[
                            ...
                            <motion.div
                                ...
                                className={`p-6 rounded-3xl border backdrop-blur-xl transition-all ${isDark ? 'bg-[#0D211A]/80 border-[#143D33] hover:bg-[#0D211A]' : 'bg-neutral-50 border-neutral-200 hover:bg-neutral-100'}`}
                            >
                ...
                <div className="relative h-[600px] flex items-center justify-center z-20">
                    <div className={`relative w-full max-w-md p-8 rounded-[2rem] border overflow-hidden
                        ${isDark ? 'bg-[#0D211A] border-[#738D7D] shadow-[0_0_50px_rgba(20,61,51,0.15)]' : 'bg-white border-[#143D33]/20 shadow-2xl'}
                    `}>
                        {/* Status Header */}
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    ...
                                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 border-2 
                                        ${isDark ? 'bg-[#143D33] border-[#738D7D] text-white' : 'bg-neutral-100 border-[#143D33] text-black'}`}
                                    >
                                        {React.createElement(messageStates[activeIndex].icon, { size: 28 })}
                                    </div>

                                    {/* Stage Label */}
                                    <h4 className={`text-[10px] md:text-xs font-pixel uppercase tracking-widest mb-4 opacity-70 ${isDark ? 'text-[#738D7D]' : 'text-[#143D33]'}`}>
                                    ...
                                    <div className={`w-full p-4 rounded-xl border text-center font-mono text-sm break-all
                                        ...
                                        bg-[#0D211A] border-[#143D33]/50 text-green-400 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]
                                    `}>
                                    ...
                                    {activeIndex === 1 || activeIndex === 2 ? (
                                        <motion.div 
                                            ...
                                            className={`h-1 mt-6 rounded-full ${isDark ? 'bg-[#738D7D]' : 'bg-[#143D33]'}`}
                                        />
                                    ) : (
