...
export default function FeaturesShowcase() {
    ...
    return (
        <section className={`py-0 pt-12 px-6 md:px-20 overflow-hidden ${isDark ? 'bg-[#0D211A]' : 'bg-white'}`}>
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 items-end">
                    ...
                    <div className="lg:col-span-7 space-y-12">
                        ...
                        <h2 className={`text-4xl md:text-6xl lg:text-7xl font-serif italic tracking-tight leading-[0.9] ${isDark ? 'text-white' : 'text-[#0D211A]'}`}>
                            Navigated by <br />
                            <span className="not-italic font-sans font-black text-transparent bg-clip-text bg-gradient-to-r from-[#738D7D] to-[#143D33] font-pixel">Intelligence.</span>
                        </h2>
                    ...
                    <div className="lg:col-span-5 relative h-[600px] flex items-center justify-center">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-100">
                            ...
                            <div
                                className="relative w-96 h-96 rounded-full border-[2px] border-[#738D7D]/40"
                                style={{ animation: 'spin-slow 20s linear infinite' }}
                            >
                                <div className="absolute -top-1.5 left-1/2 w-3 h-3 bg-[#738D7D] rounded-full shadow-[0_0_10px_rgba(20,61,51,0.5)]" />
                            </div>

                            {/* Circle 2 - Outer Ring with Marker */}
                            <div
                                className="absolute w-[500px] h-[500px] rounded-full border-[1px] border-[#738D7D]/30"
                                style={{ animation: 'spin-slow 30s linear infinite reverse' }}
                            >
                                <div className="absolute top-1/2 -right-1.5 w-3 h-3 bg-[#738D7D]/50 rounded-full" />
                            </div>

                            {/* Gradient Center - Enhanced Glow */}
                            <div className="absolute w-64 h-64 bg-[#738D7D]/10 rounded-full blur-3xl animate-pulse" />
                            <div className="absolute w-48 h-48 bg-[#738D7D]/5 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '1s' }} />
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col items-center justify-center h-full space-y-6">
                            {["Designers", "Engineers", "Founders", "Innovators"].map((role, i) => (
                                <div key={i} className={`
                                    ...
                                    ${isDark ? 'text-white' : 'text-[#0D211A]'}
                                    hover:scale-110 hover:text-[#738D7D] hover:text-glow-apex
                                 `}>
                                    {role}
                                </div>
                            ))}
