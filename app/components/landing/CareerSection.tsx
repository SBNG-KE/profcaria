...
export default function CareerSection() {
    ...
    return (
        <section className={`relative min-h-screen py-32 flex flex-col justify-center px-4 sm:px-8 md:px-24 z-10`}>
            <div className={`absolute inset-0 z-0 ${isDark ? 'bg-[radial-gradient(circle_at_center,rgba(237,62,179,0.05)_0%,transparent_70%)]' : 'bg-[radial-gradient(circle_at_center,rgba(0,167,181,0.05)_0%,transparent_70%)]'}`} />
            ...
            <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col lg:flex-row items-center gap-20">
                ...
                >
                    <h2 className={`text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none ${isDark ? 'text-white' : 'text-black'}`}>
                        Career<br />
                        <span className="font-pixel text-[#ED3EB3]">Management.</span><br />
                        Evolved.
                    </h2>
                    <p className={`text-xl leading-relaxed ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}>
                        Keep your work history, applications, documents, certificates, and opportunities together. Profcaria is more than a place to find your next job—it is a private home for your working life.
                    </p>
                ...
                >
                    {[
                        ...
                        <div key={idx} className={`p-8 rounded-3xl border flex flex-col justify-between transition-all
                            ${isDark ? 'bg-[#061D20]/50 border-[#104B52] hover:bg-[#104B52]/40 hover:-translate-y-1' : 'bg-neutral-50/80 border-neutral-200 hover:bg-white hover:-translate-y-1 hover:shadow-xl'}
                        `}>
                            <div className="mb-8">
                                <span className={`text-[10px] md:text-xs font-pixel uppercase tracking-[0.2em] ${isDark ? 'text-[#ED3EB3]' : 'text-[#104B52]/50'}`}>
                                    {item.num} {'//'} {item.tag}
                                </span>
                            </div>
                            <div>
