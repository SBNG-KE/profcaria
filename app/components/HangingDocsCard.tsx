                ...
                <div className="flex min-h-full items-start justify-center p-4 md:p-8">
                    ...
                    <div
                        className={`
                            ...
                            ${isDark
                                ? 'bg-[#061D20] shadow-2xl border-[#104B52]/50 text-white'
                                : 'bg-white shadow-2xl border-[#104B52]/20 text-[#061D20]'}
                        `}
                        style={{
                    ...
                    >
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${isDark ? 'bg-[#061D20] hover:bg-[#104B52] text-white' : 'bg-[#F7FCFC] hover:bg-[#E8F1F2] text-[#061D20]'}
                            `}
                        >
                            <X size={18} />
                        </button>
                        ...
                        <div className="relative z-10 mt-4 md:mt-6 space-y-16">
                            ...
                            <div className="text-center">
                                <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tight leading-none mb-4">
                                    <span className={`font-pixel ${isDark ? 'text-[#ED3EB3]' : 'text-[#104B52]'}`}>Profcaria</span>{' '}
                                    <span className="opacity-40">Docs</span>
                                </h2>
                                <p className={`text-sm font-medium ${isDark ? 'text-neutral-400' : 'text-[#006B76]'}`}>
                                    Everything you need to know about the platform
                                </p>
                                <div className={`w-16 h-1 mt-6 mx-auto ${isDark ? 'bg-[#ED3EB3]' : 'bg-[#104B52]'}`}></div>
                            </div>

                            {/* Sections */}
                            {sections.map((section, idx) => (
                                <div key={idx} className="space-y-5">
                                    <h3 className={`text-xl md:text-2xl font-black uppercase tracking-wide font-pixel ${isDark ? 'text-[#ED3EB3]' : 'text-[#104B52]'}`}>
                                        {section.title}
                                    </h3>

                                    {section.content && (
                                        <p className={`text-sm md:text-base leading-relaxed font-medium ${isDark ? 'text-neutral-300' : 'text-[#006B76]'}`}>
                                            {section.content}
                                        </p>
                                    )}
                                        ...
                                        <ul className="space-y-3">
                                            {section.items.map((item, i) => (
                                                <li key={i} className="flex items-start gap-3">
                                                    <div className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${isDark ? 'bg-[#ED3EB3]' : 'bg-[#104B52]'}`} />
                                                    <span className={`text-sm md:text-base leading-relaxed ${isDark ? 'text-neutral-400' : 'text-[#006B76]'}`}>
                                                        {item}
                                                    </span>
                                                </li>
                                            ))}
                                    ...
                                    {idx < sections.length - 1 && (
                                        <div className={`h-px mt-8 ${isDark ? 'bg-[#104B52]/30' : 'bg-[#104B52]/10'}`} />
                                    )}
                                </div>
                            ))}
