...
export default function VerificationClient({ id }: { id: string }) {
  ...
  return <main className="min-h-dvh bg-[#081812] p-5 text-[#F4F0E6] sm:p-10"><div className="mx-auto max-w-3xl rounded-[34px] border border-white/10 bg-[#081812] p-7 sm:p-10"><ShieldCheck className="text-[#738D7D]" /><p className="mt-8 text-xs font-black uppercase tracking-[0.28em] text-[#738D7D]">Profcaria work verification</p><h1 className="mt-3 font-serif text-5xl">Confirm only what you know.</h1>{record && <div className="mt-8 rounded-[26px] bg-[#081812] p-6"><p className="text-sm text-white/50">Person</p><p className="mt-1 text-xl font-bold">{record.workerName}</p><p className="mt-5 text-sm text-white/50">Employment presented</p><p className="mt-1 text-lg font-bold">{record.role} · {record.organization}</p><p className="mt-2 text-sm text-white/65">{record.startedAt} — {record.endedAt || 'Present'}</p><textarea value={note} onChange={event => setNote(event.target.value)} rows={4} className="mt-6 w-full rounded-2xl bg-white/5 p-4 outline-none" placeholder="Optional clarification, discrepancy or reason" /><div className="mt-4 grid gap-2 sm:grid-cols-3"><button onClick={() => respond('confirmed')} className="rounded-2xl bg-[#738D7D] p-3 text-sm font-black text-black">Confirm</button><button onClick={() => respond('partially_confirmed')} className="rounded-2xl border border-white/20 p-3 text-sm font-black">Partly confirm</button><button onClick={() => respond('declined')} className="rounded-2xl border border-red-500 p-3 text-sm font-black text-red-400">Cannot confirm</button></div></div>}{notice && <p className="mt-6 rounded-2xl bg-white/5 p-4 text-sm">{notice}</p>}<p className="mt-8 text-xs leading-6 text-white/40">This page does not create an Profcaria account and exposes only the role and dates the person asked you to verify.</p></div></main>;
}
