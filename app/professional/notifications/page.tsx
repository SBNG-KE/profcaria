...
function ChatContent() {
    ...
    return (
        <div className="h-full w-full p-1.5 md:p-3 pb-24 md:pb-3">
            <div className={`flex h-full w-full overflow-hidden font-sans rounded-3xl border shadow-2xl ${isDark ? 'bg-[#0D211A]/90 backdrop-blur-2xl border-neutral-800/50 text-neutral-200' : 'bg-white/90 backdrop-blur-2xl border-neutral-200/50 text-neutral-800'}`}>
                {/* ── LEFT SIDEBAR ── */}
                                        ...
                                        <div>
                                            ...
                                            {platformConnections.map((conn: any) => (
                                                ...
                                                >
                                                    ...
                                                    <div className={`px-2 py-1 rounded-full text-[9px] font-bold uppercase ${conn.status === 'active' ? 'bg-[#738D7D]/10 text-[#738D7D]' : isDark ? 'bg-neutral-800 text-neutral-500' : 'bg-neutral-100 text-neutral-500'}`}>
                                                        {conn.status}
                                                    </div>
                                                </button>
                                            ))}
                                                    ...
                                                    <div className={`max-w-[85%] md:max-w-[70%] ${isMe ? 'items-end' : 'items-start'}`}>
                                                        <div className={`px-4 py-3 rounded-2xl relative ${isMe ? (isDark ? 'bg-white text-black rounded-br-sm' : 'bg-black text-white rounded-br-sm') : (isDark ? 'bg-neutral-800 text-neutral-200 rounded-bl-sm' : 'bg-neutral-100 text-neutral-800 rounded-bl-sm')}`}>
                                                            ...
                                                            <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                                ...
                                                                {isMe && <CheckCheck size={12} className={msg.is_read ? "text-[#738D7D]" : (isDark ? 'text-neutral-400' : 'text-neutral-300')} />}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
