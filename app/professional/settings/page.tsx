...
export default function ProfessionalSettingsPage() {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-8 pb-32">
            <header className={`flex items-center justify-between border-b pb-8 ${isDark ? 'border-slate-800' : 'border-neutral-200'}`}>
                <div className="text-left">
                    <h1 className={`text-4xl font-black uppercase tracking-tighter ${isDark ? 'text-white' : 'text-black'}`}>Settings</h1>
                    <p className={`mt-2 ${isDark ? 'text-slate-400' : 'text-neutral-500'}`}>Manage your security and billing.</p>
                </div>
            </header>

            {/* Tabs */}
            <div className={`flex space-x-2 p-1 rounded-xl w-fit border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-neutral-100 border-neutral-200'}`}>
                <button
                    onClick={() => setActiveTab('security')}
                    className={`flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-bold transition-all ${activeTab === 'security' ? (isDark ? 'bg-white text-black' : 'bg-black text-white') + ' shadow-lg' : isDark ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-neutral-500 hover:text-black hover:bg-black/5'}`}
                >
                    <Shield size={16} /> Security
                </button>
                <button
                    onClick={() => setActiveTab('billing')}
                    className={`flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-bold transition-all ${activeTab === 'billing' ? (isDark ? 'bg-white text-black' : 'bg-black text-white') + ' shadow-lg' : isDark ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-neutral-500 hover:text-black hover:bg-black/5'}`}
                >
            ...
            {activeTab === 'security' ? (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                    {/* Password Change */}
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-neutral-200'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-2">
                                <label className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-neutral-500'}`}>Current Password</label>
                                <input
                                    type="password"
                            ...
                            <div className="space-y-2">
                                <label className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-neutral-500'}`}>New Password</label>
                                <input
                                    type="password"
                                    value={newPassword}
                    ...
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-neutral-200'}`}>
                        ...
                        <div className={`overflow-hidden rounded-xl border ${isDark ? 'border-slate-800' : 'border-neutral-200'}`}>
                            <table className={`w-full text-left text-sm ${isDark ? 'text-slate-400' : 'text-neutral-600'}`}>
                                <thead className={`font-bold uppercase text-xs tracking-wider ${isDark ? 'bg-slate-900 text-slate-200' : 'bg-neutral-50 text-neutral-700'}`}>
                                    <tr>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                    ...
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-[#0f172a] border-slate-800' : 'bg-white border-neutral-200'}`}>
                        ...
                        <div className={`p-12 rounded-2xl border-2 border-dashed text-center ${isDark ? 'border-slate-700 bg-slate-900/30' : 'border-neutral-300 bg-neutral-50'}`}>
