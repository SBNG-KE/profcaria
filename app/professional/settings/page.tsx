...
export default function ProfessionalSettingsPage() {
    ...
    // 2FA State — always enabled, user just selects method
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-8 pb-32">
            ...
            {message && (
                <div className={`p-4 rounded-2xl text-center font-bold text-sm animate-in fade-in slide-in-from-top-4 ${message.type === 'success' ? 'bg-[#738D7D]/10 text-[#738D7D] border border-[#738D7D]/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                    {message.text}
                </div>
            )}
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <h3 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-black'}`}>
                            <Lock className="text-[#738D7D]" size={24} /> Change Password
                        </h3>
                                ...
                                <div key={field.label} className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input type={field.show ? 'text' : 'password'} value={field.value} onChange={(e) => field.setter(e.target.value)}
                                            className={`w-full border rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 transition-all font-bold ${isDark ? 'bg-neutral-900/50 border-neutral-700/50 text-white focus:ring-[#738D7D]/50' : 'bg-neutral-50 border-neutral-200 text-black focus:ring-black/20'}`}
                                        />
                    ...
                    {/* 2FA Section — Always Active */}
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="flex items-center justify-between">
                            <h3 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-black'}`}>
                                <Smartphone className="text-[#738D7D]" size={24} /> Multi-Factor Authentication
                            </h3>
                            <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-[#738D7D]/10 text-[#738D7D] border border-[#738D7D]/20">
                                Verified
                            </span>
                        </div>
                        <p className={`text-sm ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}>
                            Multi-factor authentication is always active on your account. Choose your preferred verification method for sign-in.
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button
                                ...
                                className={`p-4 rounded-xl border-2 text-left transition-all ${mfaMethod === 'email'
                                    ? (isDark ? 'border-[#738D7D] bg-[#738D7D]/10' : 'border-[#738D7D] bg-[#738D7D]/5')
                            ...
                            >
                                <div className="flex items-center gap-3 mb-2">
                                    <Mail size={20} className={mfaMethod === 'email' ? 'text-[#738D7D]' : isDark ? 'text-neutral-500' : 'text-neutral-400'} />
                                    <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-black'}`}>Email OTP</span>
                                    {mfaMethod === 'email' && <CheckCircle size={16} className="text-[#738D7D] ml-auto" />}
                                </div>
                            ...
                            >
                                <div className="flex items-center gap-3 mb-2">
                                    <Smartphone size={20} className={mfaMethod === 'authenticator' ? 'text-[#738D7D]' : isDark ? 'text-neutral-500' : 'text-neutral-400'} />
                                    <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-black'}`}>Authenticator App</span>
                                    {mfaMethod === 'authenticator' && <CheckCircle size={16} className="text-[#738D7D] ml-auto" />}
                                </div>
                                <p className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>Use Google Authenticator or similar app for codes</p>
                            </button>
                        </div>

                        <div className={`flex items-center gap-3 p-4 rounded-xl border ${isDark ? 'border-[#738D7D]/30 bg-[#738D7D]/5' : 'border-[#738D7D]/20 bg-[#738D7D]/5'}`}>
                            <div className="w-10 h-10 rounded-full flex items-center justify-center bg-[#738D7D]/20 text-[#738D7D]">
                                <Shield size={20} />
                            </div>
                            <div className="flex-1">
                                <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-black'}`}>Status: Always Active</p>
                                <p className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>
                                    Current method: {mfaMethod === 'email' ? 'Email OTP' : 'Authenticator App'}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`border p-8 rounded-[32px] space-y-6 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="flex items-center justify-between">
                            <h3 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-black'}`}>
                                <Mail className="text-[#738D7D]" size={24} /> Email Login Approval
                            </h3>
                            <button
                                onClick={() => setEmailApprovalEnabled(!emailApprovalEnabled)}
                                className={`relative w-12 h-6 rounded-full transition-all duration-300 ${emailApprovalEnabled ? 'bg-[#738D7D]' : isDark ? 'bg-neutral-700' : 'bg-neutral-300'}`}
                            >
                                    ...
                                    <div key={login.id} className={`flex items-center gap-4 p-4 rounded-xl border ${isDark ? 'border-amber-500/20 bg-amber-500/5' : 'border-amber-200 bg-amber-50'}`}>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#738D7D] text-white hover:bg-[#3D6254] transition-all"
                                            >
                                                Approve
                                            </button>
                                            <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className={`border p-6 rounded-[24px] flex flex-col relative overflow-hidden group transition-all duration-300 ${isDark ? 'bg-neutral-900 border-neutral-800 hover:border-neutral-700' : 'bg-white border-neutral-200 hover:border-neutral-300 shadow-sm'}`}>
                                ...
                                <div className={`mt-8 pt-6 border-t ${isDark ? 'border-neutral-800' : 'border-neutral-100'}`}>
                                    <div className={`w-full py-3 font-bold rounded-xl text-center text-[10px] uppercase tracking-widest cursor-default ${isDark ? 'bg-[#738D7D]/20 text-[#9FB5A8]' : 'bg-[#738D7D]/10 text-[#738D7D]'}`}>
                                        Current Plan
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Plus Tier */}
                            <div className={`border-2 p-6 rounded-[24px] flex flex-col relative overflow-hidden group transition-all duration-300 transform md:-translate-y-2 shadow-lg ${isDark ? 'bg-gradient-to-b from-neutral-900 to-black border-[#738D7D]/50' : 'bg-white border-[#738D7D]'}`}>
                                <div className={`absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-[#738D7D] to-[#9FB5A8]`} />
                                <div className="absolute top-4 right-4 bg-gradient-to-r from-[#738D7D] to-[#9FB5A8] text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-md">
                                    Popular
                                </div>
                                <div className="space-y-4 flex-1 mt-2">
                                    <h4 className={`font-black text-xl flex items-center gap-2 ${isDark ? 'text-white' : 'text-[#738D7D]'}`}>
                                        Plus
                                    </h4>
                                    <div className={`text-3xl font-black ${isDark ? 'text-white' : 'text-[#738D7D]'}`}>
                                        {formatCurrency(pricing.plus)}
                                        <span className={`text-xs font-bold ml-1 ${isDark ? 'text-neutral-400' : 'text-[#738D7D]/70'}`}>/mo</span>
                                    </div>
                                    <div className="pt-2 space-y-3">
                                        <div className={`flex items-center gap-2 text-xs font-medium ${isDark ? 'text-white' : 'text-black'}`}>
                                            <CheckCircle size={14} className="text-[#738D7D] shrink-0" /> Unlimited Career AI
                                        </div>
                                        <div className={`flex items-center gap-2 text-xs font-medium ${isDark ? 'text-white' : 'text-black'}`}>
                                            <CheckCircle size={14} className="text-[#738D7D] shrink-0" /> Basic Interview Prep
                                        </div>
                                        <div className={`flex items-center gap-2 text-xs font-medium ${isDark ? 'text-white' : 'text-black'}`}>
                                            <CheckCircle size={14} className="text-[#738D7D] shrink-0" /> Resume Analyzer
                                        </div>
                                    </div>
                                </div>
                                <div className={`mt-8 pt-6 border-t ${isDark ? 'border-neutral-800' : 'border-neutral-100'}`}>
                                    <button className="w-full py-3 bg-[#738D7D] hover:bg-[#3D6254] text-white font-bold rounded-xl text-center text-[10px] uppercase tracking-widest transition-all shadow-md active:scale-95">
                                        Upgrade to Plus
                                    </button>
                                </div>
                            </div>
                        ...
                        {nextTier ? (
                            <div>
                                ...
                                <div className={`w-full h-3 rounded-full overflow-hidden ${isDark ? 'bg-neutral-800' : 'bg-neutral-200'}`}>
                                    <div className={`h-full rounded-full transition-all duration-1000 ease-out ${nextTier.key === 'gold' ? 'bg-gradient-to-r from-[#738D7D] to-[#9FB5A8]' : nextTier.key === 'blue' ? 'bg-gradient-to-r from-blue-500 to-blue-400' : 'bg-gradient-to-r from-neutral-500 to-neutral-400'}`} style={{ width: `${progressPercent}%` }} />
                                </div>
                        ...
                        {tiers.map((tier) => {
                            ...
                            return (
                                <div key={tier.key} className={`relative p-6 rounded-3xl border ${isDark ? `bg-neutral-900/50 ${borderColor}` : `bg-white border-neutral-200`} ${activeRing} transition-all`}>
                                    {isCurrent && <div className="absolute top-0 right-0 bg-[#738D7D] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-bl-xl rounded-tr-3xl">Current</div>}
