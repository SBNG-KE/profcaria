                    ...
                    <div className="relative" ref={locationDropdownRef}>
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all text-xs font-bold ${locationFilter !== 'all'
                                ? isDark
                                    ? 'bg-[#738D7D]/10 border-[#738D7D]/30 text-[#9FB5A8]'
                                    : 'bg-[#738D7D]/10 border-[#738D7D]/30 text-[#738D7D]'
                                : isDark
                                    ...
                                    {(['all', 'remote', 'onsite', 'hybrid'] as const).map(loc => (
                                        ...
                                        >
                                            <span className="flex items-center gap-2">
                                                ...
                                                {loc === 'onsite' && <span className="w-2 h-2 rounded-full bg-[#738D7D]" />}
                                                {loc === 'hybrid' && <span className="w-2 h-2 rounded-full bg-[#143D33]" />}
                                ...
                                <div className="p-5 md:p-8 space-y-6 flex-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                                ...
                                                <>
                                                    <button
                                                        ...
                                                        className={`p-2 rounded-xl border transition-all z-20 ${copiedJobId === job.id
                                                            ? 'bg-[#738D7D]/10 border-[#738D7D]/20 text-[#738D7D]'
                                                            : isDark
                                                                ? 'bg-neutral-800 border-neutral-700 text-neutral-500 hover:text-[#738D7D] hover:border-[#738D7D]/20'
                                                                : 'bg-neutral-100 border-neutral-200 text-neutral-400 hover:text-[#738D7D] hover:border-[#738D7D]/20'}`}
                                            ...
                                            <div className="px-3 py-1 bg-[#738D7D]/10 text-[10px] font-black text-[#738D7D] uppercase tracking-widest rounded-lg border border-[#738D7D]/20 flex items-center gap-2"><Zap size={10} /> Active</div>
                                        </div>
                                    </div>
                                    {job.applicationStatus && (
                                        <div className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 w-fit ${job.applicationStatus === 'pending' ? 'bg-[#738D7D]/10 text-[#738D7D] border-[#738D7D]/20' : job.applicationStatus === 'shortlisted' ? (isDark ? 'bg-white/10 text-white border-white/20' : 'bg-black/10 text-black border-black/20') : ['employed', 'hired', 'accepted'].includes(job.applicationStatus) ? 'bg-[#738D7D]/10 text-[#738D7D] border-[#738D7D]/20' : job.applicationStatus === 'rejected' ? 'bg-red-500/10 text-red-500 border-red-500/20' : isDark ? 'bg-neutral-800 text-neutral-500 border-neutral-700' : 'bg-neutral-100 text-neutral-500 border-neutral-200'}`}>
                                ...
                                <div className={`px-5 md:px-8 py-4 md:py-5 border-t transition-colors ${isDark ? 'bg-neutral-900/50 border-neutral-800 group-hover:bg-white/5' : 'bg-neutral-50 border-neutral-200 group-hover:bg-neutral-100'}`}>
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        <div className={`flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-neutral-500' : 'text-neutral-500'}`}>
                                            ...
                                            <span className="flex items-center gap-1.5"><Briefcase size={12} className="text-[#738D7D]" /> {job.employment_type ? job.employment_type.replace('-', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) : 'Full Time'}</span>
