...
export async function resolveOrganizationAccountType(accountId: string): Promise<OrganizationAccountType> {
  const { data, error } = await supabaseAdmin.schema('profcaria').from('account_identities')
...
export async function addAutomaticOrganizationAccess(input: {
  ...
  const { data: groups, error } = await supabaseAdmin.schema('profcaria').from('work_groups')
    .select('id, conversation_id').eq('organization_id', input.organizationId)
    .eq('auto_membership', true).is('archived_at', null);
  if (error) throw error;
  if (!groups?.length) return;
  const now = new Date().toISOString();
  const { error: groupError } = await supabaseAdmin.schema('profcaria').from('work_group_members').upsert(
...
export async function removeOrganizationAccess(organizationId: string, userId: string) {
  ...
  const { error: groupError } = await supabaseAdmin.schema('profcaria').from('work_group_members')
  ...
  const { error: conversationError } = await supabaseAdmin.schema('profcaria').from('conversation_members')
