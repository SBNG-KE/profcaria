...
import type { ProfcariaSession } from '@/lib/profcaria-auth';

export type ProfcariaContact = {
  id: string;
...
async function attachUsernames(contacts: ProfcariaContact[]) {
  if (!contacts.length) return contacts;
  const { data } = await supabaseAdmin.schema('profcaria').from('accounts')
...
export async function getProfcariaContacts(session: ProfcariaSession): Promise<ProfcariaContact[]> {
  ...
  return attachUsernames((people ?? []).map((person: any) => ({
    id: person.id,
    type: 'professional',
    name: `${decryptData(person.enc_first_name) || ''} ${decryptData(person.enc_last_name) || ''}`.trim() || 'Profcaria member',
    avatarUrl: decryptData(person.enc_profile_image_url) || null,
  })));
}

export async function resolveProfcariaPeople(rows: Array<{ user_id: string; account_type: string }>) {
...
export async function resolveProfcariaAccounts(accountIds: string[]) {
  const ids = [...new Set(accountIds.filter(Boolean))];
  const result = new Map<string, { name: string; avatarUrl: string | null; identityType: 'professional' | 'employer' | 'account' }>();
  if (!ids.length) return result;

  const { data: accounts } = await supabaseAdmin.schema('profcaria').from('accounts')
  ...
  const { data: identities } = await supabaseAdmin.schema('profcaria').from('account_identities')
  ...
  ids.forEach(id => {
    if (!result.has(id)) result.set(id, { name: 'Profcaria member', avatarUrl: null, identityType: 'account' });
  });
  return result;
}
