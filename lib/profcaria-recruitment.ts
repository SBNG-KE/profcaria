...
export function makeJobCode() {
  return `PRF-${new Date().getUTCFullYear()}-${randomBytes(4).toString('hex').toUpperCase()}`;
}
...
export async function addStageEvent(input: {
  ...
  const { error } = await supabaseAdmin.schema('profcaria').from('application_stage_events').insert({
...
export async function evaluateApplication(applicationId: string) {
  const { data: application, error } = await supabaseAdmin.schema('profcaria').from('applications')
  ...
  if (!policy || policy.mode === 'off') {
    await supabaseAdmin.schema('profcaria').from('applications').update({
  ...
  const { error: evaluationError } = await supabaseAdmin.schema('profcaria').from('application_ai_evaluations').insert({
  ...
  const { error: updateError } = await supabaseAdmin.schema('profcaria').from('applications').update({
    status: nextStatus,
...
export async function ensureCompanyGroupMembership(input: {
  ...
  const { data: group } = await supabaseAdmin.schema('profcaria').from('work_groups')
