...
/**
 * Mirrors a verified legacy employment transition into Profcaria. This is kept
 * idempotent so legacy retries cannot create duplicate organisations or groups.
 */
export async function syncProfcariaEmployment(input: EmploymentSyncInput) {
  const { data: existingOrganization } = await supabaseAdmin.schema('profcaria').from('organizations')
    .select('id').eq('legacy_company_id', input.companyId).maybeSingle();

  let organizationId = existingOrganization?.id as string | undefined;
  if (!organizationId) {
    const { data, error } = await supabaseAdmin.schema('profcaria').from('organizations').insert({
      legacy_company_id: input.companyId,
      name: input.companyName,
      created_by: input.companyId,
    }).select('id').single();
    if (error || !data) throw error || new Error('Unable to create Profcaria organisation');
    organizationId = data.id;
  }

  await supabaseAdmin.schema('profcaria').from('organization_members').upsert({
  ...
  const { data: existingGroup } = await supabaseAdmin.schema('profcaria').from('work_groups')
  ...
  if (!group) {
    const { data: conversation, error: conversationError } = await supabaseAdmin.schema('profcaria').from('conversations').insert({
    ...
    await supabaseAdmin.schema('profcaria').from('conversation_members').insert({
      conversation_id: conversation.id, user_id: input.companyId, account_type: 'employer', role: 'owner', membership_status: 'accepted',
    });
    const { data: createdGroup, error: groupError } = await supabaseAdmin.schema('profcaria').from('work_groups').insert({
  ...
  await supabaseAdmin.schema('profcaria').from('work_group_members').upsert({
  ...
  if (group.conversation_id) {
    await supabaseAdmin.schema('profcaria').from('conversation_members').upsert({
  ...
  await supabaseAdmin.schema('profcaria').from('employment_records').upsert({
    user_id: input.professionalId,
    organization_id: organizationId,
