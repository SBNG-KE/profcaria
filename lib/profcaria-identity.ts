...
export async function ensureProfcariaAccount(input: {
  id: string;
  ...
  const { error: accountError } = await supabaseAdmin.schema('profcaria').from('accounts').upsert(account, { onConflict: 'id' });
  if (accountError) throw accountError;

  const { error: identityError } = await supabaseAdmin.schema('profcaria').from('account_identities').upsert({
    account_id: input.id,
  ...
  const { error: preferenceError } = await supabaseAdmin.schema('profcaria').from('account_preferences').upsert({
    account_id: input.id,
  ...
  if (input.security) {
    ...
    const { error: securityError } = await supabaseAdmin.schema('profcaria').from('account_security').upsert({
      account_id: input.id,
...
export async function syncProfcariaSecurity(accountId: string, security: {
  requires2fa?: boolean;
  hasPasskey?: boolean;
  hasTotp?: boolean;
  hasEmailOtp?: boolean;
  ...
  const { error } = await supabaseAdmin.schema('profcaria').from('account_security').upsert(changes, { onConflict: 'account_id' });
