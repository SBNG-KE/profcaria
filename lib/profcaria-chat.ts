...
export async function createAttachmentUrl(storagePath: string, bucket = 'profcaria-documents') {
  const { data } = await supabaseAdmin.storage.from(bucket).createSignedUrl(storagePath, 60 * 10);
...
export async function isConversationBlocked(conversationId: string, userId: string) {
  ...
  const [blockedByMe, blockedByThem] = await Promise.all([
    supabaseAdmin.schema('profcaria').from('blocked_accounts').select('id').eq('blocker_id', userId).in('blocked_id', otherIds).limit(1),
    supabaseAdmin.schema('profcaria').from('blocked_accounts').select('id').in('blocker_id', otherIds).eq('blocked_id', userId).limit(1),
  ]);
